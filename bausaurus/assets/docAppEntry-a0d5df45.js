import { i as inBrowser, a as isProd, p as pathFromLocation, g as globalStyle, b as button, d as classNames, t as toPropsAndChildren, h as header, f as footer, c as createContext, m as mountApp } from "./utils-2e94108d.js";
...
function drillDownMenu(context2, options) {
  const { bau, css, window } = context2;
  const { base = "" } = options;
  const renderHeaderDefault = ({ currentTree, data, onclickBack }) => header2(
    Button(
      {
  ...
  const renderMenuItemDefault = ({ name, href }) => Button(
    {
      href: `${base}${href}`
    },
    name
          ...
          (subTree) => li(
            {
              class: () => classNames(
                ...
                isActive({ pathname: pathnameState.val, subTree }) && "is-active"
              ),
