...
`)}
  ...
  `,y=i.state(["HTML"]),q=i.derive(()=>y.val.length>0),N=u=>()=>{y.val.findIndex(k=>k===u)==-1&&y.val.push(u)},P=()=>y.val=[],L=u=>()=>{const w=y.val.findIndex(k=>k===u);w>=0&&y.val.splice(w,1)},I=i.state(Ae),D=i.derive(()=>y.val.length>0?I.val.filter(u=>y.val.every(w=>[...u.tools,...u.languages].includes(w))):I.val),H=()=>j({class:c`
          background: none;
          border: none;
          color: var(--color-primary);
          cursor: pointer;
        ...
        `,type:"button",onclick:P},"Clear"),R=()=>()=>_({style:()=>`visibility:${q.val?"visible":"hidden"}`,class:c`
          margin-top: -50px;
          min-height: 70px;
          background-color: white;
          padding: 1rem;
        ...
        `},i.loop(y,l(),u=>p(C(u),j({type:"button",onclick:L(u)},C("❌")))),H()),V=u=>l({class:c`
          flex-grow: 1;
          list-style: none;
          display: inline-flex;
        ...
        `},[...u.tools,...u.languages].map(w=>p(j({type:"button",onclick:N(w)},w)))),G=u=>u.new?C({class:["badge","badge-new"]},"New!"):"",K=u=>u.featured?C({class:["badge","badge-featured"]},"Featured"):"",B=u=>p({id:u.id,class:u.featured?"featured":""},F({class:"container"},b({src:u.logo,alt:"",width:88,height:88}),F({class:c`
              display: grid;
              gap: 0.7rem;
            `},F({class:c`
                display: inline-flex;
              ...
              `},[u.postedAt,u.contract,u.location].join(" • "))),V(u)));return()=>f({class:M},R(),i.loop(D,l({class:"job-list"}),B))}function Fe(e){const{bau:i,css:c}=e,{header:f}=i.tags,l=c`
    height: 156px;
