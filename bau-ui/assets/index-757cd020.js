...
`);function pt(e,t={}){const{bau:n,css:o}=e,{div:a,li:i}=n.tags,s=o`
    ...
    ${fr()}
  `;return function(...c){let[{size:l=t.size??"md",variant:u=t.variant??"outline",color:d=t.color??"neutral",label:p,placeholder:m,Option:b,options:f,getOptionLabel:h=({label:Y})=>Y,onSelect:w=()=>{},id:x,required:C,name:A,loading:k,...I},...T]=W(c);const M=dt(e),B=V(e),j=Se(e,{variant:u,color:d,size:l}),Z=Ce(e),ne=Me(e,{variant:u,color:d,size:l}),L=n.state(I.value);n.derive(()=>{L.val&&(ae.value=h(L.val),w(L.val))});const U=n.state(""),H=n.state(!1),y=n.state(0),g=()=>{H.val=!1},v=n.state(f),S=Y=>se=>Y.val&&h(se)==h(Y.val),E=()=>{ve.openDialog(),H.val=!0,U.val="",v.val=f,y.val=f.findIndex(S(L));const Y=pe.querySelector("li.selected");Y&&(Y.scrollIntoView({block:"center"}),ee.scrollIntoView({block:"end"}))},P=()=>{ve.closeDialog(),H.val=!1,U.val="",y.val=0},$=Y=>{const{value:se}=Y.target;U.val=se,se?v.val=f.filter(me=>h(me).match(new RegExp(`${se}`,"i"))):v.val=f},K=Y=>{ve.open?P():E()},R=({option:Y,index:se})=>me=>{L.val=Y,y.val=se,P()},_=()=>{const Y=pe.querySelector("li.active");Y&&Y.scrollIntoView({block:"center",behavior:"smooth"})},q=Y=>{switch(console.log("onkeydown",Y.key,y.val),Y.key){case"Escape":P();break;case"ArrowDown":y.val<v.val.length-1?y.val++:y.val=0,_();break;case"ArrowUp":y.val<=0?y.val=v.val.length-1:y.val--,_();break;case"Enter":L.val=v.val[y.val],U.val="",P(),Y.preventDefault();break}},Q=B({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":H,"aria-label":p,onclick:K,variant:u,color:d,size:l,class:k==!0&&"loading",disabled:k},()=>L.val?h(L.val):p,()=>k==!0&&ne({visibility:k})),ee=j({value:U,placeholder:m,autofocus:!0,type:"search",autocomplete:"off",autocapitalize:"none",spellcheck:!1,role:"combobox","aria-autocomplete":"list","aria-expanded":H,oninput:$,onkeydown:q,...I}),ae=j({class:o`
        width: 1px;
        opacity: 0;
        left: 0;
        bottom: 0;
        position: absolute;
      `,tabindex:-1,value:U,required:C,name:A}),pe=a({class:D(u,d,l,"content")},ee,()=>Z({class:D(u,d,l)},v.val.map((Y,se)=>i({class:()=>D(y.val==se&&"active",S(L)(Y)&&"selected"),onclick:R({option:Y,index:se})},b(Y))))),ve=M({id:x,triggerEl:Q,contentEl:pe,onClose:g,class:o`
        overflow: hidden;
...
`,rs={title:"Checkbox",package:"checkbox",description:"The checkbox component uses the native input date type.",sourceCodeUrl:"https://github.com/grucloud/bau/blob/main/bau-ui/checkbox/checkbox.js",importStatement:'import checkbox from "@grucloud/bau-ui/checkbox";',examples:[{title:"Default",description:"A simple checkbox.",code:as,createComponent:os}],gridItem:vn},ss=e=>{const t=G(e);return()=>t(rs)},is=e=>{const{bau:t}=e,{section:n,div:o}=t.tags,a=ut(e),i=V(e),s=()=>i("Header"),r=()=>o("Content");return()=>n(a({Header:s,Content:r}))},cs=`import button from "@grucloud/bau-ui/button";
...
`);function Ve(e,t={}){const{bau:n,css:o}=e,{div:a,li:i,select:s,option:r}=n.tags,c=V(e),l=dt(e),u=Ce(e),d=o`
    ...
    ${bi()}
  `;return function(...m){let[{size:b=t.size??"md",variant:f=t.variant??"outline",color:h=t.color??"neutral",label:w,Option:x,options:C,getOptionLabel:A=({label:E})=>E,...k},...I]=W(m);const T=n.state(k.value),M=n.state(!1),B=n.state(0),j=()=>{v.openDialog(),v.focus(),M.val=!0},Z=()=>{v.closeDialog(),M.val=!1},ne=()=>{M.val=!1},L=E=>{M.val?Z():j(),E.preventDefault()},U=({option:E,index:P})=>$=>{T.val=A(E),S.value=T.val,S.setCustomValidity(""),B.val=P,Z(),$.preventDefault()},H=E=>{switch(E.preventDefault(),E.key){case"Escape":Z();break;case"ArrowDown":B.val<C.length-1?B.val++:B.val=0;break;case"ArrowUp":B.val<=0?B.val=C.length-1:B.val--;break;case"Enter":M.val?(T.val=A(C[B.val]),Z()):j();break}},y=()=>u({tabindex:"0",class:D(h,f)},C.map((E,P)=>i({class:()=>D(B.val==P&&"active"),onclick:U({option:E,index:P})},x(E)))),g=c({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":M,"aria-label":w,onclick:L,color:h,variant:f,size:b},()=>!T.val&&w,T),v=l({triggerEl:g,contentEl:y(),onClose:ne}),S=s(k,r({value:""},"--Select Category--"),C.map(E=>r(A(E))));return S.value=k.value,a({...k,class:D("select",h,b,d,t==null?void 0:t.class,k==null?void 0:k.class),onkeydown:H},S,g,v)}}const In=(e,t)=>{const{bau:n,css:o}=e,{div:a,span:i}=n.tags,s=Ve(e,t),r=[{code:"AD",label:"Andorra",phone:"376"},{code:"AE",label:"United Arab Emirates",phone:"971"},{code:"AF",label:"Afghanistan",phone:"93"}],c=l=>a({class:o`
          display: flex;
          justify-content: space-between;
          gap: 0.5rem;
