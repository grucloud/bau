...
`);function at(e,t={}){const{bau:n,css:o,window:a}=e,{input:i}=n.tags,s=u=>{a.document.documentElement.setAttribute("data-theme",u),localStorage.setItem("theme",u)},r=()=>{try{return localStorage.getItem("theme")}catch{}},c=r();c?s(c):a.matchMedia("(prefers-color-scheme: dark)").matches?s("dark"):a.matchMedia("(prefers-color-scheme: light)").matches?s("light"):s(yo);const l=o`
  ...
  `,K=({children:R,pathnameState:y,variant:m,color:b,size:S})=>A({class:M(m,b,S)},R.map(E=>v({class:()=>M(E.children&&"has-children",h({pathname:y.val,subTree:E})&&"active")},f({variant:m,color:b,size:S,subTree:E})))),ee=({variant:R,color:y,size:m,currentTree:b,pathnameState:S})=>{const{children:E,parentTree:O,data:$,renderList:G}=b;return console.log("Menu"),x({class:M("drillDownMenu",R,y,m)},O&&g({variant:R,color:y,size:m,data:$,currentTree:b}),E&&G?G({renderListDefault:K,children:E,pathnameState:S,variant:R,color:y,size:m}):K({children:E,pathnameState:S,variant:R,color:y,size:m}))},V=({tree:R,pathname:y})=>{let m=Pt({})({...R}),b=Ot(y)(m);return b||(b=m),b},J=n.state(a.location.pathname.replace(c,""));return console.log("addEventListener onclick"),a.document.addEventListener("click",R=>{console.log("window onclick");const{target:y}=R,m=y.getAttribute("href");if(y.tagName==="A"&&m&&!m.startsWith("http")){let b=m.replace(c,"");r||(b=b.replace(y.hash,"")),J.val=b}}),function(y){const{size:m=t.size??"md",variant:b=t.variant??"plain",color:S=t.color??"neutral",tree:E,...O}=y;let $,G=n.derive(()=>($=V({tree:E,pathname:J.val}),$)),L=1;const _=Q=>{console.log("onclick");const{dataset:ae}=Q.target;ae.buttonback=="true"?L=-1:ae.ischild=="false"?L=1:ae.ischild=="true"&&(L=0)},X=Q=>{switch(Q){case 1:return`${D} ${Re}`;case-1:return`${B} ${Re}`;default:return""}},ne=Q=>{switch(Q){case 1:return`${B} ${Re} reverse`;case-1:return`${D} ${Re} reverse`;default:return""}};return w({class:M(z,b,S,m,t==null?void 0:t.class,O.class),onclick:_},k({animationHide:()=>X(L),animationShow:()=>ne(L)},n.bind({deps:[G],render:()=>()=>ee({variant:b,color:S,size:m,currentTree:$,pathnameState:J})})))}}const To=()=>te.map(e=>`
&.input.${e} {
...
`);function ve(e,t={}){const{bau:n,css:o}=e,{input:a}=n.tags,i=o`
    ...
    ${To()}
  `;return function(r){const{variant:c=t.variant??"outline",color:l=t.color??"neutral",...u}=r;return a({type:"text",...u,class:M("input",t.class,t.size??"md",l,c,i,u.class)})}}function _t(e){const{tr:t,bau:n,css:o,config:a,states:i,window:s}=e,{div:r,ul:c,li:l,nav:u,a:d,span:p}=n.tags,g=ve(e,{variant:"plain",color:"neutral"}),h={data:{name:"Root"},children:[{data:{name:"Home",href:"/"}},{data:{name:"Getting Started",href:"/GettingStarted"}},{data:{name:"Components",href:"/components"},renderList:({renderListDefault:x,children:C,pathnameState:I,variant:k,color:A,size:T})=>{const D=n.state(""),B=n.derive(()=>D.val==""?C:C.filter(V=>V.data.name.match(new RegExp(`${D.val}`,"i")))),z=V=>{D.val=V.target.value};getComputedStyle(document.documentElement).getPropertyValue("--color-danger");const ee=`url('data:image/svg+xml,<svg fill="${getComputedStyle(document.documentElement).getPropertyValue(`--color-${A}`)}" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512"><path d="M416 208c0 45.9-14.9 88.3-40 122.7L502.6 457.4c12.5 12.5 12.5 32.8 0 45.3s-32.8 12.5-45.3 0L330.7 376c-34.4 25.2-76.8 40-122.7 40C93.1 416 0 322.9 0 208S93.1 0 208 0S416 93.1 416 208zM208 352a144 144 0 1 0 0-288 144 144 0 1 0 0 288z"/></svg>')`;return r({class:o`
          display: flex;
          flex-direction: column;
          & .input {
            margin: 0.5rem 1rem;
