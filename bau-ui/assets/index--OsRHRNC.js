...
`);function ie(t,e={}){const{bau:n,css:o,window:a}=t,{input:r}=n.tags,i=u=>{a.document.documentElement.setAttribute("data-theme",u),localStorage.setItem("theme",u)},s=()=>{try{return localStorage.getItem("theme")}catch{}},c=s();c?i(c):a.matchMedia("(prefers-color-scheme: dark)").matches?i("dark"):a.matchMedia("(prefers-color-scheme: light)").matches?i("light"):i(No);const l=o`
  ...
  `,z=({children:w,pathnameState:k,variant:v,color:x,size:E})=>D({class:[v,x,E]},w.map(M=>f({class:()=>[M.children&&"has-children",g({pathname:k.val,subTree:M})&&"active"]},p({variant:v,color:x,size:E,subTree:M})))),G=({variant:w,color:k,size:v,currentTree:x,pathnameState:E})=>{const{children:M,parentTree:P,data:J,renderList:I}=x;return h({class:["drillDownMenu",w,k,v]},P&&b({variant:w,color:k,size:v,data:J,currentTree:x}),M&&I?I({renderListDefault:z,children:M,pathnameState:E,variant:w,color:k,size:v}):z({children:M,pathnameState:E,variant:w,color:k,size:v}))},H=({tree:w,pathname:k})=>{let v=Ge({})({...w}),x=Fe(k)(v);return x||(x=v),x},j=({target:w})=>{let k=`${w.closest("a").pathname.replace(".md","").replace(c,"")}${w.hash}`;return s||(k=k.replace(w.hash,"")),k};return function(k){const{size:v=e.size??"md",variant:x=e.variant??"plain",color:E=e.color??"neutral",tree:M,...P}=k,J=n.state(a.location.pathname.replace(c,""));let I=H({tree:M,pathname:J.val});const _=n.state(JSON.stringify(I.data));let R;a.document.addEventListener("click",Z=>{const{target:ot}=Z,st=ot.closest("a");if(!st)return;const rt=st.getAttribute("href");rt&&!rt.startsWith("http")&&!rt.startsWith("#")&&!rt.startsWith("?")&&(I=H({tree:M,pathname:j(Z)}),_.val=JSON.stringify(I.data),J.val=j({target:ot}))});const W=Z=>{const{buttonback:ot,ischild:st}=Z.target.dataset;ot=="true"?R=-1:st=="false"?R=1:st=="true"&&(R=0)},tt=Z=>{switch(Z){case 1:return`${N} ${Ht}`;case-1:return`${B} ${Ht}`;default:return""}},X=Z=>{switch(Z){case 1:return`${B} ${Ht} reverse`;case-1:return`${N} ${Ht} reverse`;default:return""}};return y({class:[$,x,E,v,e==null?void 0:e.class,P.class],onclick:W},T({animationHide:()=>tt(R),animationShow:()=>X(R)},n.bind({deps:[_],render:()=>()=>G({variant:x,color:E,size:v,currentTree:I,pathnameState:J})})))}}const Lo=()=>nt.map(t=>`
&.input.${t} {
...
`);function sc(t,e={}){const{bau:n,css:o}=t,{div:a,li:r,select:i}=n.tags,s=O(t),c=zt(t),l=xt(t),u=St(t,{color:"neutral",variant:"outline"}),d=o`
    ...
    ${rc()}
  `;return function(...b){let[{size:p=e.size??"md",variant:g=e.variant??"outline",color:f=e.color??"neutral",name:y,label:h,Option:C,options:S,defaultValue:T=[],getOptionLabel:D,getOptionValue:A,renderValue:N,onSelect:B=()=>{},loading:$,...z},G]=V(b);const H=bt(t,{variant:g,color:f,size:p}),j=n.state(T),w=n.state(!1),k=n.state(0),v=()=>{W.openDialog(),W.focus(),w.val=!0},x=()=>{W.closeDialog(),w.val=!1},E=()=>{w.val=!1},M=X=>{W.open?x():v(),X.preventDefault()},P=()=>Array.from(tt.selectedOptions).map(({value:X})=>S.find(Z=>A(Z)==X)),J=X=>{switch(X.preventDefault(),X.key){case"Escape":x();break;case"ArrowDown":k.val<S.length-1?k.val++:k.val=0;break;case"ArrowUp":k.val<=0?k.val=S.length-1:k.val--;break;case"Enter":if(W.open){const Z=X.currentTarget.querySelectorAll("input")[k.val];Z.checked=!Z.checked;const ot=tt.options[k.val+1];ot.selected=!ot.selected,j.val=P()}else v();break}},I=X=>Z=>{const ot=[...tt.options].find(({value:st})=>st==A(X));Z.target.checked?ot.selected=!0:ot.selected=!1,j.val=P()},_=()=>l({tabindex:"0",class:[f,g]},S.map((X,Z)=>r({class:()=>k.val==Z&&"active"},n.tags.label(u({checked:T.find(ot=>A(ot)==A(X)),name:`${y}-${A(X)}`,onchange:I(X)}),C(X))))),R=s({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":w,"aria-label":h,onclick:M,color:f,variant:g,size:p,class:$==!0&&"loading",disabled:$},()=>j.val.length?N(j.val):h,()=>$==!0&&H({visibility:$})),W=c({triggerEl:R,contentEl:_(),onClose:E}),tt=i({name:y,multiple:!0,...z},n.tags.option({value:""},"--Category--"),S.map(X=>n.tags.option({value:A(X),selected:T.find(Z=>A(Z)==A(X))},D(X))));return a({...z,class:["multiSelect",f,p,d,e==null?void 0:e.class,z==null?void 0:z.class],onkeydown:J},tt,R,W)}}const ic=t=>{const{bau:e,css:n}=t,{div:o,span:a,form:r,footer:i}=e.tags,s=sc(t),c=O(t,{variant:"outline",color:"neutral"}),l=At(t,{size:"sm"}),u=[{group:"EC2"},{group:"ECS"},{group:"IAM"},{group:"Lambda"},{group:"RDS"},{group:"S3"}],d=m=>a(m.group);return()=>r({onsubmit:b=>{b.preventDefault();const{selectedOptions:p}=b.target.elements.myMultiSelect,g=Array.from(p).map(({value:f})=>f);alert(JSON.stringify(g))},class:n`
          display: inline-flex;
          flex-direction: column;
          gap: 1rem;
