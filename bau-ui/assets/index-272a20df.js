...
`}function dt(){const e=document.getElementById("loading");e&&e.classList.add("m-fadeOut")}let me=e=>Object.prototype.toString.call(e??0).slice(8,-1),ut=e=>me(e)=="Object",we=e=>me(e)=="Function",se=e=>["Object","Array"].includes(me(e)),ye=Object.getPrototypeOf,le=e=>ee(e)?e.val:e,ee=e=>e==null?void 0:e.__isState,mt=["splice","push","pop","shift","unshift","sort","reverse"],ae=(e,t)=>{const n=new Array(e.length);for(let o=0;o<e.length;o++)n[o]=t(e[o],o);return n};const U=e=>!ee(e[0])&&ut(e[0])?e:[{},...e];function ht(e){let t=(e==null?void 0:e.window)??window,{document:n}=t,o,r=new Set,a=new Set,s=!1,l,i=f=>n.createElement(f),m=(f,x,k)=>{let C=l;l=x;let T=f(k);return l=C,T},d=()=>{o||(o=window.requestAnimationFrame(()=>{r.forEach(f=>{f.bindings=f.bindings.filter(x=>{var k;return(k=x.element)==null?void 0:k.isConnected}),!f.bindings.length&&!f.computed&&r.delete(f)}),o=void 0}))},c=(f,x,k,C,T,_)=>{var P;if(s){a.add(f);return}for(let X of f.bindings){let{deps:N,element:L,renderInferred:R,render:G,renderItem:V}=X;if(V&&x)(P=g(L,C,(...K)=>b(V(...K)),k,T,_)[x])==null||P.call();else{let K=R?R({element:L}):G({element:L,renderItem:V})(...N.map(le));K!==L&&L.replaceWith(X.element=b(K))}}E(f),d()},u=(f,x,k=[])=>({get(C,T,_){var P;if(l==null||l.add(f),T==="_isProxy")return!0;if(!((P=C[T])!=null&&P._isProxy)&&!ee(C[T])&&se(C[T]))C[T]=new Proxy(C[T],u(f,x,[...k,T]));else if(mt.includes(T)){let X=C[T];return(...N)=>{let L=X.apply(C,N);return c(f,T,L,N,x,k),L}}return Reflect.get(C,T,_)},set(C,T,_,P){let X=Reflect.set(C,T,_,P);return c(f,"setItem",X,{prop:T,value:_},x,[...k,T]),X}}),p=(f,x)=>new Proxy(x,u(f,x)),g=(f,x,k,C,T,_)=>{let P=()=>f.replaceChildren(...ae(C,k)),X=N=>f[N]&&f.removeChild(f[N]);return{assign:P,sort:P,reverse:P,setItem:()=>{var L;let N=_[0];(L=f.children[N])==null||L.replaceWith(k(T[N],N))},push:()=>f.append(...ae(x,(N,L)=>k(N,T.length+L))),unshift:()=>f.prepend(...ae(x,k)),pop:()=>X("lastChild"),shift:()=>X("firstChild"),splice:()=>{let[N,L,...R]=x;const{length:G}=f.children;for(let V=N>=0?Math.min(N+L-1,G-1):G-1;V>=(N>=0?N:G+N);V--)f.children[V].remove();if(R.length){let V=R.forEach((K,re)=>k(K,N+re));f.children[N]?f.children[N].after(...V):f.append(...V)}}}},h=f=>({oldVal:f,bindings:[],listeners:[],__isState:!0,get val(){let x=this;return l==null||l.add(x),x.valProxy??(x.valProxy=se(f)?p(x,f):f,x.valProxy)},set val(x){let k=this,C=k.val;se(x)?(k.valProxy=p(k,x),c(k,"assign",x)):x!==C&&(k.valProxy=x,c(k)),k.oldVal=C}}),b=f=>f==null||f===!1?i("span"):f.nodeType?f:n.createTextNode(f),v=(f,x)=>{let k=new Set;return x.val=m(f,k),k},y=f=>{let x=h(),k=v(f,x);x.computed=!0;for(let C of k)C.listeners.push({computed:f,deps:k,state:x});return x},E=f=>{for(let x of[...f.listeners])v(x.computed,x.state)},S=(f,...x)=>{if(x.length){let k=[];for(let C of x.flat(1/0))C!=null&&k.push(ee(C)?H({deps:[C],render:()=>T=>T}):we(C)?D({renderInferred:C}):b(C));f.append(...k)}},I={},z=(f,x)=>f&&(Object.getOwnPropertyDescriptor(f,x)??z(ye(f),x)),A=(f,x,k)=>{var C;return I[f+","+x]??(I[f+","+x]=((C=z(k,x))==null?void 0:C.set)??0)},B=(f,x)=>new MutationObserver((k,C)=>{k.filter(T=>T.removedNodes).forEach(T=>[...T.removedNodes].find(_=>_===f&&(x({element:f}),C.disconnect(),!0)))}).observe(f.parentNode,{childList:!0}),M=f=>new Proxy(function(k,...C){var X;let[T,..._]=U(C),P=f?n.createElementNS(f,k):i(k);for(let[N,L]of Object.entries(T)){if(N.startsWith("bau"))continue;let R=A(k,N,ye(P))?G=>P[N]=G:G=>P.setAttribute(N,G);L==null||(ee(L)?H({deps:[L],render:()=>()=>(R(L.val),P)}):we(L)&&(!N.startsWith("on")||L.isDerived)?D({renderInferred:()=>(R(L({element:P})),P)}):L.renderProp?H({deps:L.deps,render:()=>()=>(R(L.renderProp({element:P})(...L.deps.map(le))),P)}):R(L))}return S(P,..._),(X=T.bauCreated)==null||X.call(T,{element:P}),T.bauMounted&&t.requestAnimationFrame(()=>T.bauMounted({element:P})),T.bauUnmounted&&t.requestAnimationFrame(()=>B(P,T.bauUnmounted)),P},{get:(x,k)=>x.bind(void 0,k)}),O=(f,x,k)=>{f.element=b(k);for(let C of x)ee(C)&&(r.add(C),C.bindings.push(f));return f.element},D=({renderInferred:f,element:x})=>{let k=new Set,C=m(f,k,{element:x});return O({renderInferred:f},k,C)},H=({deps:f,element:x,render:k,renderItem:C})=>O({deps:f,render:k,renderItem:C},f,k({element:x,renderItem:C})(...f.map(le))),j=(f,x,k)=>H({deps:[f],render:({renderItem:C})=>T=>(x.append(...ae(T,C)),x),renderItem:k}),W=f=>{s=!0,f(),s=!1,a.forEach(c),a.clear()};return{tags:M(),tagsNS:M,state:h,bind:H,loop:j,derive:y,stateSet:r,batch:W}}const pt=e=>{let t=0,n=11;for(;t<e.length;)n=101*n+e.charCodeAt(t++)>>>0;return"bau"+n},bt=(e,t,n,o)=>{const r=e.createElement("style");r.id=n,r.append(o),(t??e.head).append(r)},gt=(e,t)=>e.reduce((n,o,r)=>n+o+(t[r]??""),"");function ft(e){let{document:t}=(e==null?void 0:e.window)??window;const n=o=>(r,...a)=>{const s=gt(r,a),l=pt(s);return!t.getElementById(l)&&bt(t,e==null?void 0:e.target,l,o(l,s)),l};return{css:n((o,r)=>`.${o} { ${r} }`),keyframes:n((o,r)=>`@keyframes ${o} { ${r} }`),createGlobalStyles:n((o,r)=>r)}}function vt(e){return{bau:ht(),...ft(),tr:n=>n,window,...e}}function $(...e){return e.filter(t=>t).join(" ")}function Y(e,t){const{bau:n,css:o}=e,r={root:o`
    ...
    `,a:o``,disabled:o`
      ...
      pointer-events: none;
      box-shadow: none;
    `};return function(...s){let[{color:l,variant:i="outline",size:m,disabled:d,href:c,...u},...p]=U(s);return(c?n.tags.a:n.tags.button)({...u,class:$(r.root,i,m,l,c?r.a:r.button,d&&r.disabled,t==null?void 0:t.class,u.class),disabled:d,href:c,...!c&&{type:"button"}},p)}}function wt(e){const{tr:t,bau:n,css:o,config:r}=e,{i:a,header:s,h1:l,div:i,a:m,img:d,b:c,ul:u,li:p}=n.tags,{svg:g,path:h}=n.tagsNS("http://www.w3.org/2000/svg"),b=n.state(!0),v=Y(e),y=()=>a({class:o`
          color: var(--font-color-inverse);
...
`,{hideToLeft:t`
 ...
 `});function he(e,t){const{bau:n,css:o,window:r}=e,{base:a=""}=t,s=({currentTree:D,data:H,onclickBack:j})=>b(E({href:`${a}${D.parentTree.children[0].data.href}`,onclick:j({currentTree:D}),class:o`
  ...
  `;return function(...b){let[{color:v,variant:y="outline",size:E,content:S,...I},...z]=U(b);const A=B=>{const{Header:M,Content:O,name:D}=B;return l({class:$(v,y,E),onclick:u(D)},m({class:()=>$(c.val==D&&"active")},d({type:"button","aria-controls":`bau-${D}`,"aria-expanded":({element:H})=>c.val==D},M(B))),a({class:"content",role:"region",id:`bau-${D}`,"data-state":({element:H})=>{const j=c.val==D;return p({element:H,open:j}),j}},O(B)))};return a({class:$("accordion",g,t==null?void 0:t.class,I.class)},s(r.map(A)))}}const J=["neutral","primary","success","danger","warning"],$t=["plain","outline","solid"],F=e=>{const{bau:t,css:n}=e,{div:o,table:r,tbody:a,tr:s,td:l,thead:i,th:m}=t.tags;return function({Item:c,name:u}){return o({class:n`
          overflow-x: auto;
          overflow-y: visible;
          width: 100%;
          max-width: 80vw;
        ...
        `},r(i(s(m(u??"Variant/Color"),J.map(p=>m(p)))),a($t.map(p=>s(m(p),J.map((g,h)=>l(c({color:g,variant:p},{index:h}))))))))}},At=e=>{const{tr:t,bau:n,css:o}=e,{article:r,div:a,h3:s,h2:l,h1:i,p:m}=n.tags,d=F(e),c=(...g)=>a({class:o`
...
`};function oe(e,t){const{bau:n,css:o,createGlobalStyles:r}=e,{div:a}=n.tags;Mt({css:o,createGlobalStyles:r});const s=o`
 ...
 `}),Nt=e=>{const{bau:t}=e,{section:n,div:o,h1:r}=t.tags,a=Te(),s=Y(e),l=Pt(e);return function(){const i=t.state(!0),m=o(),d=c=>{m.replaceChildren(a({parent:m,animationHide:`${l.hideRight} 0.5s`,animationShow:`${l.showRight} 0.5s`},o(c.val?"Ciao":"")))};return d(i),n({id:"animate"},o(r("Test Animate"),o(s({onclick:()=>{i.val=!i.val,d(i)}},()=>i.val?"Hide":"Show")),m))}};function Be(e,t){const{bau:n}=e,{span:o,img:r}=n.tags,a=n.state(!0),s=n.state(!1),l=()=>a.val=!1,i=m=>{a.val=!1,s.val=!0};return function(...d){let[{color:c,variant:u="outline",size:p,width:g=60,height:h=60,...b},...v]=U(d);return o({class:$(t==null?void 0:t.class,b.class)},()=>a.val?"Loading...":"",()=>s.val&&"Error",r({width:g,height:h,onload:l,onerror:i,class:$(t==null?void 0:t.class,c,u,p,b.class),...b}))}}const Lt=e=>{const{tr:t,bau:n,css:o}=e,{section:r,h2:a,h3:s}=n.tags,l=o`
  ...
  `,p=n.state(""),g=n.state(""),h=n.state(!1),b=n.state(0),v=()=>{h.val=!1};return function(...E){let[{variant:S="outline",color:I,size:z,id:A,label:B,placeholder:M,Option:O,options:D,getOptionLabel:H=({label:G})=>G,...j},...W]=U(E);const f=n.state(D),x=()=>{R.openDialog(),h.val=!0,g.val="",f.val=D},k=()=>{R.closeDialog(),h.val=!1,g.val=""},C=G=>{const{value:V}=G.target;g.val=V,V?f.val=D.filter(K=>H(K).match(new RegExp(`${V}`,"i"))):f.val=D},T=G=>{console.log("onclickButton",h.val),h.val?k():x()},_=G=>V=>{p.val=H(G),k()},P=G=>{switch(G.key){case"Escape":k();break;case"ArrowDown":b.val++;break;case"ArrowUp":b.val--;break;case"Enter":p.val=H(f.val[b.val]),g.val="",k();break}},X=d({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":h,"aria-label":B,onclick:T,class:$(S,I,z)},()=>!p.val&&B,p),N=a({id:A,value:g,placeholder:M,autofocus:!0,type:"search",autocomplete:"new-password",autocapitalize:"none",spellcheck:!1,role:"combobox","aria-autocomplete":"list","aria-expanded":h,oninput:C,onkeydown:P,class:$(S,I,z)}),R=c({id:A,triggerEl:X,contentEl:(()=>r({class:$(S,I,z,"content")},N,()=>s(f.val.map((G,V)=>l({class:()=>$(b.val==V&&"active"),onclick:_(G)},O(G))))))(),onClose:v});return r({...j,class:$("autocomplete",u,t==null?void 0:t.class,j==null?void 0:j.class)},X,R)}}const Ht=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,span:i}=n.tags,m=(...g)=>a({class:o`
          display: flex;
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
  ...
  `;return function(...l){let[{color:i,variant:m="outline",size:d,content:c,...u},...p]=U(l);return r({...u,class:$("badge",a,t==null?void 0:t.class,u==null?void 0:u.class)},r({class:$(i,m,d)},c),...p)}}const jt=e=>{const{bau:t,css:n}=e,{section:o,div:r,h3:a,h2:s}=t.tags,l=(...c)=>r({class:n`
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
    ...
    `});return()=>o({id:"badge"},s("Badge"),a("Basic Badge"),l(m({content:"10"},"☏")),a("Badges Table"),i({Item:(c,{index:u})=>m({...c,content:`${u*100}`},"☏")}),a("Badge custom"),l(d({content:"1"},"☏")))};function ze(e,t){const{bau:n,css:o}=e,{ul:r,li:a,a:s,span:l}=n.tags,i=o`
    list-style: none;
    display: flex;
    align-items: center;
    padding-left: 0;
  ...
  `;return function(...d){let[{color:c,variant:u="outline",size:p,items:g,...h},...b]=U(d);return r({...h,class:$(i,t==null?void 0:t.class,h==null?void 0:h.class)},g.map(({href:v,name:y})=>a((v?s:l)({href:v,class:$(c,u,p)},y))))}}const Ot=e=>{const{tr:t,bau:n}=e,{section:o,h2:r,h3:a}=n.tags,s={items:[{href:"/",name:"⌂"},{name:"Dir"},{href:"/dir/subdir",name:"SubDir"}]},l=F(e),i=ze(e);return()=>o({id:"breadcrumbs"},r(t("Breadcrumbs")),a("Bacis Breadcrumb"),i(s),a("Breadcrumbs Table"),l({Item:m=>i({...m,...s})}))},Gt=e=>{const{bau:t,css:n}=e,{section:o,p:r,h3:a}=t.tags,s=F(e),l=Y(e);return()=>o({id:"button",class:n`
          & button {
            margin: 0.5rem;
          }
...
`).join(`
`))()}
  `;return function(...i){let[{color:m="neutral",variant:d="plain",size:c,...u},...p]=U(i);return r({...u,type:"date",class:$("calendar",s,m,d,c,t==null?void 0:t.class,u==null?void 0:u.class)},...p)}}const Vt=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,label:i}=n.tags,m=F(e),d=(...g)=>a({class:o`
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
  ...
  `;return function(...l){let[{size:i,variant:m="outline",color:d="neutral",onclick:c,...u},...p]=U(l);return r({...u,onclick:c,class:$("chip",a,i,m,d,c&&"clickable",t==null?void 0:t.class,u==null?void 0:u.class)},...p)}}const Ut=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l}=n.tags,i=F(e),m=(...c)=>a({class:o`
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
  ...
  `;return function(...l){let[{color:i,variant:m="outline",size:d,...c},...u]=U(l);return r({type:"checkbox",required:"required",...c,class:$(a,i,m,d,t==null?void 0:t.class,c==null?void 0:c.class)})}}const _t=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,label:s,h2:l,form:i}=n.tags,m=F(e),d=Le(e),c=n.state(!1),u=n.state(!1),p=h=>b=>{h.val=!!b.target.checked},g=(...h)=>a({class:o`
        ...
        `},...h);return()=>r({id:"checkbox"},i(l(t("Checkbox Examples")),g(d({id:"myCheckbox",name:"myCheckbox",checked:c,onchange:p(c)}),s({for:"myCheckbox"},"My Checkbox")),g(d({id:"myCheckbox-disabled",disabled:!0,name:"myCheckbox-disabled",checked:u,onchange:p(u)}),s({for:"myCheckbox-disabled"},"My Disabled Checkbox")),l(t("Checkbox Table")),m({Item:(h,{index:b})=>d({id:`myCheckbox-${b}`,name:`myCheckbox-${b}`,...h})})))};function Xt(e,t){const{bau:n,css:o}=e,{div:r}=n.tags,a=o`
    position: fixed;
    top: 80px;
    left: 0px;
    z-index: 2;
  ...
  `;return function(...l){let[{color:i,variant:m="outline",size:d,openState:c,...u},...p]=U(l);return r({class:$(a,t==null?void 0:t.class,u.class)},r({class:()=>$("overlay",c.val&&"overlay-open"),onclick:()=>{c.val=!1}}),r({class:()=>$("content",c.val&&"content-open")},p))}}const Wt=e=>{const{tr:t,bau:n}=e,{section:o,h2:r}=n.tags,a=n.state(!1),s=Xt(e),l=Y(e),i=Ie(e);return()=>o({id:"drawer"},r(t("Drawer")),l({onclick:()=>{a.val=!a.val}},"OPEN DRAWER"),s({openState:a},i()))},qt=e=>{const{tr:t,bau:n,window:o,config:r}=e,{section:a,h2:s,h3:l}=n.tags,i=n.state(o.location.pathname.replace(r.base,"")),m=F(e),d={data:{name:"Root Menu"},children:[{data:{name:"Menu 1",href:"#dd-menu1"},children:[{data:{name:"Sub Menu 1",href:"#dd-menusub2"},children:[{data:{name:"Sub Sub Menu 1",href:"#menusubsub1"}}]},{data:{name:"Sub Menu 2",href:"#menusub1"}}]},{data:{name:"Menu 2",href:"#dd-menu2"},children:[{data:{name:"Sub Menu 21",href:"#menusub21"}}]},{data:{name:"Menu 3",href:"#menu3"}}]},c=he(e,{base:r.base+"/components/drillDownMenu"});return()=>a({id:"drillDownMenu"},s(t("Drill Down Menu")),c({tree:d,pathnameState:i}),l("Drill Down Table"),m({Item:u=>c({tree:d,...u})}))};function He(e,t){const{bau:n,css:o}=e,{div:r,span:a,label:s,input:l}=n.tags,i={base:o`
    ...
    `};return function(d,...c){const{variant:u="outline",color:p="neutral",size:g,Component:h,disabled:b,...v}=d;return r({class:$(i.base,b&&i.disabled,t==null?void 0:t.class,d.class)},s({class:$(u,p,g)},h({disabled:b}),l({type:"file",disabled:b,...v})),a({class:"filename-display"}))}}const Yt=e=>{const{tr:t,bau:n,css:o}=e,{svg:r,use:a}=n.tagsNS("http://www.w3.org/2000/svg"),{section:s,div:l,h3:i,h2:m,span:d}=n.tags,c=F(e),u=n.state("No file selected"),p=He(e),g=b=>{const v=b.target.files[0];v?u.val=v.name:u.val="No file selected"},h=({disabled:b})=>l({class:$(o`
            display: flex;
            align-items: center;
            flex-direction: column;
...
`))()}
  ...
  `,d=n.state(""),c=n.state(!1),u=n.state(0);return function(...g){let[{color:h="neutral",variant:b="outline",size:v,id:y,label:E,Option:S,options:I,getOptionLabel:z=({label:C})=>C,...A},...B]=U(g);const M=()=>{k.openDialog(),c.val=!0},O=()=>{k.closeDialog(),c.val=!1},D=()=>{c.val=!1},H=C=>{c.val?O():M()},j=C=>T=>{d.val=z(C),O()},W=C=>{switch(C.preventDefault(),C.key){case"Escape":O();break;case"ArrowDown":u.val<I.length-1?u.val++:u.val=0;break;case"ArrowUp":u.val<=0?u.val=I.length-1:u.val--;break;case"Enter":c.val?(d.val=z(I[u.val]),O()):M();break}},f=()=>a(I.map((C,T)=>s({class:()=>$(u.val==T&&"active"),onclick:j(C)},S(C)))),x=l({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":c,"aria-label":E,onclick:H,class:$(h,b,v)},()=>!d.val&&E,d),k=i({id:y,triggerEl:x,contentEl:f(),onClose:D});return r({...A,class:$("select",m,t==null?void 0:t.class,A==null?void 0:A.class),onkeydown:W},x,k)}}const ea=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,span:i}=n.tags,m=(...g)=>a({class:o`
          display: flex;
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
...
`).join(`
`))()};
  `;return function(...i){let[{color:m="neutral",variant:d="outline",size:c,...u},...p]=U(i);return r({...u,type:"range",class:$("slider",m,d,c,s,t==null?void 0:t.class,u.class)},...p)}}const ta=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,p:i,label:m,datalist:d,option:c,br:u}=n.tags,p=n.state(0),g=S=>{p.val=S==null?void 0:S.target.value},h=(...S)=>a({class:o`
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
        `},...S),b=F(e),v=ne(e),y=ne(e),E=ne(e);return()=>r({id:"slider"},l(t("Slider")),i("Slider value: ",p),s("Basic Slider"),h(v({oninput:g,name:"slider-simple"})),s(t("Slider Table")),b({Item:S=>v(S)}),s("Slider Min Max: -1000 1000"),h(y({oninput:g,min:-1e3,max:1e3})),s("Slider Step 20"),h(v({oninput:g,step:20,min:-100,max:100})),s("Slider Vertical"),h(a({class:o`
              display: flex;
...
`);function Fe(e,t){const{bau:n,css:o}=e,{input:r}=n.tags,a=o`
  ...
  `;return function(...l){let[{color:i="neutral",variant:m="plain",size:d,...c},...u]=U(l);return r({...c,class:$("switch",a,i,m,d,t==null?void 0:t.class,c.class),type:"checkbox",required:"required"},...u)}}const oa=e=>{const{tr:t,bau:n,css:o}=e,{section:r,form:a,label:s,div:l,h2:i}=n.tags,m=F(e),d=Fe(e);return()=>r({id:"switch"},i(t("Switch Examples")),a(l({class:o`
            ...
            `},s({for:"my-shinny-switch"},"My shinny switch"),d({id:"my-shinny-switch"}))),i(t("Switch Table")),m({Item:c=>l({class:o`
              ...
              `},s("off ",d({...c,id:`my-switch-example-off-${c.color}-${c.variant}`})),s("on ",d({...c,id:`my-switch-example-on-${c.color}-${c.variant}`,checked:!0})))}))};function de(e,t){const{bau:n,css:o}=e,{tabDefs:r}=t,{div:a,ul:s,li:l}=n.tags,i=n.state(r),m=n.state(r[0]),d=u=>i.val.find(p=>p.name==u),c={base:o`
      display: flex;
      flex-direction: column;
      & ul {
        display: flex;
    ...
    `};return function(...p){let[{color:g,variant:h="plain",size:b,...v},...y]=U(p);const E=I=>{const{Header:z,disabled:A,name:B}=I;return l({class:()=>$(m.val.name==B&&"active",A&&"disabled"),onclick:M=>M.srcElement.dispatchEvent(new CustomEvent("tab.select",{detail:{tabName:B},bubbles:!0}))},z(I))},S=a({class:$(c.base,h,b,g,t==null?void 0:t.class,v.class)},n.loop(i,s(),E),()=>m.val.Content?m.val.Content({}):"");return S.addEventListener("tab.select",I=>{var B,M;const{tabName:z}=I.detail,A=d(z);A&&((B=m.val.exit)==null||B.call(),m.val=A,(M=A.enter)==null||M.call())},!1),S.addEventListener("tab.add",I=>{var A;const{tab:z}=I.detail;(A=z.enter)==null||A.call(),i.val.push(z)},!1),S.addEventListener("tab.remove",I=>{var A;const z=i.val.findIndex(B=>B.name==I.detail.tabName);z>0&&((A=i.val[z].exit)==null||A.call(),i.val.splice(z,1))},!1),S}}const ra=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,p:i,i:m}=n.tags,d=F(e),c=Y(e),u=(...y)=>a({class:o`
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
        `},...y),p=()=>({name:"New Tab",Header:({name:y})=>a(y),Content:()=>a("My Paragraph")}),h=de(e,{tabDefs:[{name:"Tab1",Header:()=>a("TAB"),Content:()=>a(i("My Tab 1 Content"))},{name:"Tab2",Header:()=>a("TAB 2"),Content:()=>a(i("My tab 2 Content"))},{name:"Tab Disabled",disabled:!0,Header:()=>a("Tab Disabled"),Content:()=>a(i("My tab Disabled"))}]}),v=de(e,{tabDefs:[{name:"Tab1",Header:()=>a(m({class:o`
                font-size: 1rem;
                margin: 0 0.3rem;
          ...
          `})),s("Add and remove tabs"),u(v({})),l(t("Tabs Table")),d({Item:y=>h(y)}))};function Q(e,t){const{bau:n,css:o,createGlobalStyles:r}=e,{div:a}=n.tags;r`
  :root {
...
`;const s=o`
  ...
  `;return function(...i){let[{...m},...d]=U(i);return a({...m,class:$("table-container",s,t==null?void 0:t.class,m==null?void 0:m.class)},...d)}}const sa=e=>{const{bau:t,css:n}=e,{section:o,div:r,h3:a,h2:s,th:l,td:i,tr:m,table:d,thead:c,tbody:u,caption:p}=t.tags;function g(B,M,O,D,H){return{name:B,calories:M,fat:O,carbs:D,protein:H}}const h=[g("Frozen yoghurt",159,6,24,4),g("Ice cream sandwich",237,9,37,4.3),g("Eclair",262,16,24,6),g("Cupcake",305,3.7,67,4.3),g("Gingerbread",356,16,49,3.9)],b=(...B)=>r({class:n`
    ...
    `});return()=>o({id:"table"},s(m("Table")),a("Basic Table"),b(E(d(p("Basic Table"),y(),u(h.map(v))))),a("Dense Table"),b(S(d(p("Dense Table"),y(),u(h.map(v))))),a("Zebra Table"),b(I(d(p("Zebra Table"),y(),u(h.map(v))))),a("Caption Bottom"),b(z(d(p("Caption Bottom Table"),y(),u(h.map(v))))),a("Overflow Header"),b(A(d(p("Overflow Header"),y(),u(h.map(v))))))};function Re(e,t){const{bau:n,css:o}=e,{div:r}=n.tags,a=Y(e),s=be(e),l=o`
    display: flex;
    align-items: center;
    justify-content: flex-end;
  ...
  `,i=({label:p,icon:g,...h})=>a({variant:"plain","aria-label":p,title:p,...h},g),m=({count:p,totalCount:g,page:h,rowsPerPage:b})=>r({class:"pages-numbers"},Number(h-1)*Number(b)+(p>0?1:0),"-",Math.min(h*b,g)," of ",g),d=({count:p,page:g,rowsPerPage:h})=>r({class:"pages-numbers"},(g-1)*h+(p>0?1:0),"-",g*h),c=p=>p<=1,u=(p,g,h)=>p>=Math.ceil(g/h);return function(...g){let[{count:h=0,totalCount:b=0,page:v=1,rowsPerPage:y=50,onPageChange:E,isLoading:S=!1,disableFirst:I=()=>c(v),disablePrevious:z=()=>c(v),disableNext:A=()=>u(v,b,y),disableLast:B=()=>u(v,b,y),...M},...O]=U(g);const D=Math.max(0,Math.ceil(b/y)),H=E({page:1}),j=E({page:v-1}),W=E({page:v+1}),f=E({page:D});return r({...M,class:$("table-pagination",l,S&&"disabled",t==null?void 0:t.class,M==null?void 0:M.class)},s({class:"spinner",visibility:S,size:"md"}),b>0?m({count:h,totalCount:b,page:v,maxPages:D,rowsPerPage:y}):d({count:h,page:v,maxPages:D,rowsPerPage:y}),r(i({label:"First",icon:"⟪",onclick:H,disabled:I()}),i({label:"Previous",icon:"⟨",onclick:j,disabled:z()}),i({label:"Next",icon:"⟩",onclick:W,disabled:A()}),i({label:"Last",icon:"⟫",onclick:f,disabled:B()})))}}const la=(e=1e3)=>new Array(e).fill("").map(()=>({name:"my name",email:"myemail@mail.col"})),ia=e=>{const{bau:t,css:n}=e,{th:o,td:r,tr:a,table:s,thead:l,tbody:i}=t.tags,m=la(45),d=({name:y,email:E})=>a(r(y),r(E)),c=()=>l(o({class:n`
            text-align: left;
          `},"Name"),o({class:n`
            text-align: left;
          `},"Email")),u=Re(e),p=Q(e,{class:n`
      max-width: 650px;
    `}),g=t.state(m),h=t.state({count:m.length,totalCount:m.length,page:1,rowsPerPage:10}),b=t.derive(()=>g.val.slice(h.val.page*h.val.rowsPerPage,(h.val.page+1)*h.val.rowsPerPage)),v=({page:y})=>E=>{h.val.page=y};return()=>p(s(c(),()=>i(b.val.map(d))),()=>u({...h.val,onPageChange:v}))},ca=e=>{const{bau:t,css:n}=e,{th:o,td:r,tr:a,table:s,thead:l,tbody:i,div:m}=t.tags,d=t.state(!1),c=t.state([]),u=t.state(""),p=t.derive(()=>c.val.length),g=t.state(1),h=t.state(10),b=t.derive(()=>c.val),v=M=>`https://api.github.com/orgs/aws/repos?${new URLSearchParams(M).toString()}`,y=({page:M})=>O=>{g.val=M,E(v({page:M,per_page:h.val}))};E(v({page:1,per_page:h.val}));async function E(M){try{d.val=!0;const O=await fetch(M,{});if(O.ok){const D=await O.json();c.val=D;return}throw O}catch(O){u.val=O.message}finally{d.val=!1}}const S=({name:M,description:O,stargazers_count:D})=>a(r(M),r(O),r({class:n`
            text-align: right;
          `},D)),I=()=>l(o({class:n`
            text-align: left;
          `},"Name"),o({class:n`
          ...
          `},"Description"),o({class:n`
            text-align: right;
          `},"Stars")),z=Re(e),A=Q(e,{class:n`
      min-width: 650px;
    `}),B=({message:M})=>m(M);return()=>A(()=>z({rowsPerPage:h.val,page:g.val,count:p.val,totalCount:-1,isLoading:d.val,onPageChange:y,disableNext:()=>!1}),s(I(),()=>u.val&&B({message:u.val}),()=>i(b.val.map(S))))},da=e=>{const{bau:t,css:n}=e,{section:o,div:r,h3:a,h2:s,tr:l}=t.tags,i=ia(e),m=ca(e),d=(...c)=>r({class:n`
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
          & .table-container {
            width: 100%;
    ...
    `}),p=()=>a({class:o`
        ...
        `},c({side:"bottom-start",titleEl:p()},d({},"bottom start")),c({side:"bottom-centered",titleEl:p()},d({},"bottom centered")),c({side:"bottom-end",titleEl:p()},d({},"bottom end")))];return()=>r({id:"tooltip"},s(t("Tooltip")),a({class:o`
          ...
          `},g()),s(t("Tooltip custom")),a({class:o`
            ...
            flex-direction: column;
            gap: 1rem;
          `},u({titleEl:p()},d({},"custom tooltip"))),s(t("Tooltip Table")),m({Item:h=>c({titleEl:p(),...h},d({},`${h.color} ${h.variant}`))}))},ma="light";function Ve(e,t){const{bau:n,css:o,window:r}=e,{input:a}=n.tags,s=d=>{r.document.documentElement.setAttribute("data-theme",d),localStorage.setItem("theme",d)},l=()=>{try{return localStorage.getItem("theme")}catch{}},i=l();i?s(i):r.matchMedia("(prefers-color-scheme: dark)").matches?s("dark"):r.matchMedia("(prefers-color-scheme: light)").matches?s("light"):s(ma);const m=o`
    position: relative;
    width: 2rem;
    height: 2rem;
    display: flex;
  ...
  `;return function(...c){let[{color:u,variant:p="outline",size:g,...h},...b]=U(c);return a({required:"required",title:"Switch Theme",...h,class:$(m,u,p,g,t==null?void 0:t.class,h.class),type:"checkbox",checked:l()=="dark",onclick:v=>{s(v.target.checked?"dark":"light")}},...b)}}const ha=e=>{const{tr:t,bau:n,css:o}=e,{section:r,form:a,div:s,h2:l}=n.tags,i=F(e),m=Ve(e);return()=>r({id:"theme-switch"},l(t("Theme Switch")),a(s({class:o`
              display: flex;
              align-items: center;
              > * {
                margin: 0.5rem;
  ...
  `,expanded:e`
    ...
    `});function Ue(e,t){const{bau:n,css:o,createGlobalStyles:r,window:a}=e,{renderMenuItem:s}=t,{ul:l,li:i,nav:m,div:d}=n.tags,c=pa({css:o,createGlobalStyles:r}),u=({element:b,closeState:v})=>{b.scrollHeight!=0&&(v.val?p(b):g(b))};function p(b){b.style.height=b.scrollHeight+"px";const v=()=>{b.removeEventListener("transitionend",v)};b.addEventListener("transitionend",v),a.requestAnimationFrame(()=>{b.style.height="0px"})}function g(b){const v=()=>{b.removeEventListener("transitionend",v),b.style.height=null};b.addEventListener("transitionend",v),b.style.height=b.scrollHeight+"px"}const h=({depth:b=1,maxDepth:v})=>y=>{const{children:E,expanded:S}=y,I=n.state(!S);return i({class:()=>$(E?I.val?c.collapsed:c.expanded:"")},d({class:o`
        ...
        `},a({class:o`
          ...
          `},s(t("Pages Examples")),fa(e)()))}}const wa=e=>{const{bau:t,css:n,config:o}=e,{section:r,div:a,h1:s,span:l,p:i,ul:m,li:d,a:c,main:u,header:p,footer:g,label:h}=t.tags,{svg:b,use:v}=t.tagsNS("http://www.w3.org/2000/svg"),y=F(e),S=Me(e,{accordionDefs:[{name:"Item1",Header:()=>"Item 1",Content:()=>a(i("Item 1 Content"))},{name:"Item2",Header:()=>"Item 2",Content:()=>a(i("Item 2 Content"))},{name:"Item3",Header:()=>"Item 3",Content:()=>a(i("Item 3 content"))}]}),I=oe(e),z=De(e),A=[{code:"AD",label:"Andorra",phone:"376"},{code:"AE",label:"United Arab Emirates",phone:"971"},{code:"AF",label:"Afghanistan",phone:"93"}],B=w=>a({class:n`
          display: flex;
          justify-content: space-between;
          gap: 0.5rem;
    ...
    `}),O=ie(e),D={items:[{href:"/",name:"⌂"},{name:"Dir"},{href:"/dir/subdir",name:"SubDir"}]},H=ze(e),j=Y(e),W=Pe(e),f=ce(e),x=Le(e),k=Ne(e),C={data:{name:"Root Menu"},children:[{data:{name:"Menu 1",href:"#m1"},children:[{data:{name:"Sub Menu 1",href:"#menusub2"},children:[{data:{name:"Sub Sub Menu 1",href:"#menusubsub1"}}]},{data:{name:"Sub Menu 2",href:"#menusub1"}}]},{data:{name:"Menu 2",href:"#menu2"},children:[{data:{name:"Sub Menu 21",href:"#menusub21"}}]},{data:{name:"Menu 3",href:"#menu2"}}]},T=he(e,{base:o.base+"/components"}),_=({disabled:w})=>a({class:$(n`
        ...
        `},l(w.label),l(w.code)),re=ne(e),_e=be(e),fe=Fe(e),Xe=de(e,{tabDefs:[{name:"Tab1",Header:()=>a("TAB"),Content:()=>a(i("My Tab 1 Content"))},{name:"Tab2",Header:()=>a("TAB 2"),Content:()=>a(i("My tab 2 Content"))},{name:"Tab Disabled",disabled:!0,Header:()=>a("Tab Disabled"),Content:()=>a(i("My tab Disabled"))}]}),We=Ve(e),qe=()=>l("My tooltip"),Ye=ue(e),Ke={data:{name:"Root Menu"},children:[{data:{name:"Menu 1",href:"#menu"},expanded:!0,children:[{data:{name:"Sub Menu 1",href:"#menusub2"}},{data:{name:"Sub Menu 2",href:"#menusub1"}}]},{data:{name:"Menu 2",href:"#menu2"},children:[{data:{name:"Sub Menu 21",href:"#menusub21"}}]}]},Ze=Ue(e,{renderMenuItem:({name:w,href:q})=>c({href:q,onclick:Je=>{Je.preventDefault()}},w)}),ve=[{name:"Accordion",Item:w=>S({...w})},{name:"Alert",Item:w=>I({...w},`Alert ${w.color}`)},{name:"Autocomplete",Item:w=>z({...w,options:A,Option:B,getOptionLabel:({label:q})=>q,label:"Country",placeholder:"Search countries",id:"country"})},{name:"Avatar",Item:w=>M({...w,src:"./grucloud.svg",alt:"GruCloud"})},{name:"Badge",Item:(w,{index:q})=>O({...w,content:`${q*100}`},"☏")},{name:"Breadcrumbs",Item:w=>H({...w,...D})},{name:"Button",Item:w=>j({...w},`${w.variant} ${w.color}`)},{name:"Button Group",Item:w=>W({...w},j({},"ONE"),j({},"TWO"),j({},"THREE"))},{name:"Calendar",Item:w=>a({class:n`
            ...
            `},h(`${w.color} ${w.variant}`,f({...w})))},{name:"Checkbox",Item:w=>h({class:n`
              ...
              color: var(--color-content-secondary);
              padding: 0.2rem;
            `},`${w.color} ${w.variant}`,x({id:`myCheckbox-gallery-${w.color}-${w.variant}`,name:`myCheckbox-gallery-${w.color}-${w.variant}`,...w}))},{name:"Chip",Item:w=>k({...w},`Chip ${w.color}`)},{name:"DrillDown Menu",Item:w=>T({tree:C,...w})},{name:"File Input",Item:w=>P({Component:_,name:"file",accept:"text/*",onchange,...w})},{name:"Input",Item:w=>X({name:"my-input",id:"my-input-with",placeholder:"Enter text",...w})},{name:"Modal",Item:w=>{const q=R(w);return a(j({...w,onclick:()=>{q.showModal()}},"OPEN MODAL"),q)}},{name:"Select",Item:w=>a(G({...w,options:V,Option:K,getOptionLabel:({label:q})=>q,label:"Select a country..."}))},{name:"Slider",Item:w=>a({class:n`
              border: 1px dotted var(--color-emphasis-200);
              font-size: smaller;
              color: var(--color-content-secondary);
              padding: 0.2rem;
            `},h(`${w.color} ${w.variant}`,re({...w,id:`my-slider-${w.color}-${w.variant}`})))},{name:"Spinner",Item:w=>_e(w)},{name:"Switch",Item:w=>a({class:n`
              & label {
                display: inline-flex;
                border: 1px dotted var(--color-emphasis-200);
                font-size: smaller;
