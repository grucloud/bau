...
`}function dt(){const e=document.getElementById("loading");e&&e.classList.add("m-fadeOut")}let me=e=>Object.prototype.toString.call(e??0).slice(8,-1),ut=e=>me(e)=="Object",we=e=>me(e)=="Function",se=e=>["Object","Array"].includes(me(e)),ye=Object.getPrototypeOf,le=e=>ee(e)?e.val:e,ee=e=>e==null?void 0:e.__isState,mt=["splice","push","pop","shift","unshift","sort","reverse"],ae=(e,t)=>{const n=new Array(e.length);for(let o=0;o<e.length;o++)n[o]=t(e[o],o);return n};const U=e=>!ee(e[0])&&ut(e[0])?e:[{},...e];function ht(e){let t=(e==null?void 0:e.window)??window,{document:n}=t,o,r=new Set,a=new Set,s=!1,l,i=f=>n.createElement(f),m=(f,x,k)=>{let C=l;l=x;let T=f(k);return l=C,T},d=()=>{o||(o=window.requestAnimationFrame(()=>{r.forEach(f=>{f.bindings=f.bindings.filter(x=>{var k;return(k=x.element)==null?void 0:k.isConnected}),!f.bindings.length&&!f.computed&&r.delete(f)}),o=void 0}))},c=(f,x,k,C,T,_)=>{var N;if(s){a.add(f);return}for(let X of f.bindings){let{deps:z,element:L,renderInferred:R,render:G,renderItem:V}=X;if(V&&x)(N=g(L,C,(...K)=>b(V(...K)),k,T,_)[x])==null||N.call();else{let K=R?R({element:L}):G({element:L,renderItem:V})(...z.map(le));K!==L&&L.replaceWith(X.element=b(K))}}E(f),d()},u=(f,x,k=[])=>({get(C,T,_){var N;if(l==null||l.add(f),T==="_isProxy")return!0;if(!((N=C[T])!=null&&N._isProxy)&&!ee(C[T])&&se(C[T]))C[T]=new Proxy(C[T],u(f,x,[...k,T]));else if(mt.includes(T)){let X=C[T];return(...z)=>{let L=X.apply(C,z);return c(f,T,L,z,x,k),L}}return Reflect.get(C,T,_)},set(C,T,_,N){let X=Reflect.set(C,T,_,N);return c(f,"setItem",X,{prop:T,value:_},x,[...k,T]),X}}),h=(f,x)=>new Proxy(x,u(f,x)),g=(f,x,k,C,T,_)=>{let N=()=>f.replaceChildren(...ae(C,k)),X=z=>f[z]&&f.removeChild(f[z]);return{assign:N,sort:N,reverse:N,setItem:()=>{var L;let z=_[0];(L=f.children[z])==null||L.replaceWith(k(T[z],z))},push:()=>f.append(...ae(x,(z,L)=>k(z,T.length+L))),unshift:()=>f.prepend(...ae(x,k)),pop:()=>X("lastChild"),shift:()=>X("firstChild"),splice:()=>{let[z,L,...R]=x;const{length:G}=f.children;for(let V=z>=0?Math.min(z+L-1,G-1):G-1;V>=(z>=0?z:G+z);V--)f.children[V].remove();if(R.length){let V=R.forEach((K,re)=>k(K,z+re));f.children[z]?f.children[z].after(...V):f.append(...V)}}}},p=f=>({oldVal:f,bindings:[],listeners:[],__isState:!0,get val(){let x=this;return l==null||l.add(x),x.valProxy??(x.valProxy=se(f)?h(x,f):f,x.valProxy)},set val(x){let k=this,C=k.val;se(x)?(k.valProxy=h(k,x),c(k,"assign",x)):x!==C&&(k.valProxy=x,c(k)),k.oldVal=C}}),b=f=>f==null||f===!1?i("span"):f.nodeType?f:n.createTextNode(f),v=(f,x)=>{let k=new Set;return x.val=m(f,k),k},w=f=>{let x=p(),k=v(f,x);x.computed=!0;for(let C of k)C.listeners.push({computed:f,deps:k,state:x});return x},E=f=>{for(let x of[...f.listeners])v(x.computed,x.state)},S=(f,...x)=>{if(x.length){let k=[];for(let C of x.flat(1/0))C!=null&&k.push(ee(C)?H({deps:[C],render:()=>T=>T}):we(C)?D({renderInferred:C}):b(C));f.append(...k)}},I={},P=(f,x)=>f&&(Object.getOwnPropertyDescriptor(f,x)??P(ye(f),x)),A=(f,x,k)=>{var C;return I[f+","+x]??(I[f+","+x]=((C=P(k,x))==null?void 0:C.set)??0)},B=(f,x)=>new MutationObserver((k,C)=>{k.filter(T=>T.removedNodes).forEach(T=>[...T.removedNodes].find(_=>_===f&&(x({element:f}),C.disconnect(),!0)))}).observe(f.parentNode,{childList:!0}),M=f=>new Proxy(function(k,...C){var X;let[T,..._]=U(C),N=f?n.createElementNS(f,k):i(k);for(let[z,L]of Object.entries(T)){if(z.startsWith("bau"))continue;let R=A(k,z,ye(N))?G=>N[z]=G:G=>N.setAttribute(z,G);L==null||(ee(L)?H({deps:[L],render:()=>()=>(R(L.val),N)}):we(L)&&(!z.startsWith("on")||L.isDerived)?D({renderInferred:()=>(R(L({element:N})),N)}):L.renderProp?H({deps:L.deps,render:()=>()=>(R(L.renderProp({element:N})(...L.deps.map(le))),N)}):R(L))}return S(N,..._),(X=T.bauCreated)==null||X.call(T,{element:N}),T.bauMounted&&t.requestAnimationFrame(()=>T.bauMounted({element:N})),T.bauUnmounted&&t.requestAnimationFrame(()=>B(N,T.bauUnmounted)),N},{get:(x,k)=>x.bind(void 0,k)}),j=(f,x,k)=>{f.element=b(k);for(let C of x)ee(C)&&(r.add(C),C.bindings.push(f));return f.element},D=({renderInferred:f,element:x})=>{let k=new Set,C=m(f,k,{element:x});return j({renderInferred:f},k,C)},H=({deps:f,element:x,render:k,renderItem:C})=>j({deps:f,render:k,renderItem:C},f,k({element:x,renderItem:C})(...f.map(le))),O=(f,x,k)=>H({deps:[f],render:({renderItem:C})=>T=>(x.append(...ae(T,C)),x),renderItem:k}),W=f=>{s=!0,f(),s=!1,a.forEach(c),a.clear()};return{tags:M(),tagsNS:M,state:p,bind:H,loop:O,derive:w,stateSet:r,batch:W}}const pt=e=>{let t=0,n=11;for(;t<e.length;)n=101*n+e.charCodeAt(t++)>>>0;return"bau"+n},bt=(e,t,n,o)=>{const r=e.createElement("style");r.id=n,r.append(o),(t??e.head).append(r)},gt=(e,t)=>e.reduce((n,o,r)=>n+o+(t[r]??""),"");function ft(e){let{document:t}=(e==null?void 0:e.window)??window;const n=o=>(r,...a)=>{const s=gt(r,a),l=pt(s);return!t.getElementById(l)&&bt(t,e==null?void 0:e.target,l,o(l,s)),l};return{css:n((o,r)=>`.${o} { ${r} }`),keyframes:n((o,r)=>`@keyframes ${o} { ${r} }`),createGlobalStyles:n((o,r)=>r)}}function vt(e){return{bau:ht(),...ft(),tr:n=>n,window,...e}}function $(...e){return e.filter(t=>t).join(" ")}function Y(e,t){const{bau:n,css:o}=e,r={root:o`
...
`,{hideToLeft:t`
 ...
 `});function he(e,t){const{bau:n,css:o,window:r}=e,{base:a=""}=t,s=({currentTree:D,data:H,onclickBack:O})=>b(E({href:`${a}${D.parentTree.children[0].data.href}`,onclick:O({currentTree:D}),class:o`
            min-width: 3rem;
  ...
  `,M=({variant:D,color:H,size:O,onclickItem:W,onclickBack:f,currentTree:x,pathnameState:k})=>{const{children:C,parentTree:T,data:_}=x;return console.log("Menu",x,k.val),p({class:$("drillDownMenu",D,H,O)},T&&m({data:_,currentTree:x,onclickBack:f}),C&&u(C.map(N=>h({class:()=>$(N.children&&"has-children",c({pathname:k.val,subTree:N})&&"is-active"),onclick:N.children&&W({currentTree:N})},d(N.data)))))},j=({tree:D,pathnameStateInitial:H})=>{let O=$e({})(D),W=Ae(H)(O);return W||(W=O),W};return function(H){const{variant:O="plain",color:W="neutral",size:f,tree:x,pathnameState:k=n.state(r.location.pathname),...C}=H,T=k.val,_=({currentTree:L})=>R=>X(R,z,L,!0),N=({currentTree:L})=>R=>X(R,z,L.parentTree,!1),X=(L,R,G,V)=>{R.firstChild.replaceChildren(w({parent:R,animationHide:`${V?S:I} ${xe}`,animationShow:`${V?P:A} ${xe}`},M({variant:O,color:W,size:f,currentTree:G,onclickItem:_,onclickBack:N,pathnameState:k})))},z=g({class:$(B,t==null?void 0:t.class,C.class)},p(M({variant:O,color:W,size:f,currentTree:j({tree:x,pathnameStateInitial:T}),onclickItem:_,onclickBack:N,pathnameState:k})));return z}}const kt={data:{name:"Root"},children:[{data:{name:"Bau UI",href:"/"}},{data:{name:"Components",href:"/components"},children:[{data:{name:"Accordion",href:"/components/accordion"}},{data:{name:"Alert",href:"/components/alert"}},{data:{name:"Alert Stack",href:"/components/alertStack"}},{data:{name:"Animate",href:"/components/animate"}},{data:{name:"Autocomplete",href:"/components/autocomplete"}},{data:{name:"Avatar",href:"/components/avatar"}},{data:{name:"Badge",href:"/components/badge"}},{data:{name:"Breadcrumb",href:"/components/breadcrumb"}},{data:{name:"Button",href:"/components/button"}},{data:{name:"Button Group",href:"/components/buttonGroup"}},{data:{name:"Calendar",href:"/components/calendar"}},{data:{name:"Chip",href:"/components/chip"}},{data:{name:"Checkbox",href:"/components/checkbox"}},{data:{name:"Drawer",href:"/components/drawer"}},{data:{name:"DrillDown Menu",href:"/components/drillDownMenu"}},{data:{name:"File Input",href:"/components/fileInput"}},{data:{name:"Input",href:"/components/input"}},{data:{name:"Modal",href:"/components/modal"}},{data:{name:"Popover",href:"/components/popover"}},{data:{name:"Select",href:"/components/select"}},{data:{name:"Slider",href:"/components/slider"}},{data:{name:"Spinner",href:"/components/spinner"}},{data:{name:"Switch",href:"/components/switch"}},{data:{name:"Table",href:"/components/table"}},{data:{name:"Table Pagination",href:"/components/tablePagination"}},{data:{name:"Tab",href:"/components/tabs"}},{data:{name:"Tooltip",href:"/components/tooltip"}},{data:{name:"Theme Switch",href:"/components/themeSwitch"}},{data:{name:"Tree View",href:"/components/treeView"}}]}]};function Ie(e){const{tr:t,bau:n,css:o,config:r,states:a}=e,{div:s,ul:l,li:i,nav:m,a:d,span:c}=n.tags,u=he(e,{base:r.base});return function(){return s({class:o`
          grid-area: sidebar;
          position: sticky;
  ...
  `;return function(...b){let[{color:v,variant:w="outline",size:E,content:S,...I},...P]=U(b);const A=B=>{const{Header:M,Content:j,name:D}=B;return l({class:$(v,w,E),onclick:u(D)},m({class:()=>$(c.val==D&&"active")},d({type:"button","aria-controls":`bau-${D}`,"aria-expanded":({element:H})=>c.val==D},M(B))),a({class:"content",role:"region",id:`bau-${D}`,"data-state":({element:H})=>{const O=c.val==D;return h({element:H,open:O}),O}},j(B)))};return a({class:$("accordion",g,t==null?void 0:t.class,I.class)},s(r.map(A)))}}const J=["neutral","primary","success","danger","warning"],$t=["plain","outline","solid"],F=e=>{const{bau:t,css:n}=e,{div:o,table:r,tbody:a,tr:s,td:l,thead:i,th:m}=t.tags;return function({Item:c,name:u}){return o({class:n`
          overflow-x: auto;
          overflow-y: visible;
          width: 100%;
          max-width: 80vw;
...
`};function oe(e,t){const{bau:n,css:o,createGlobalStyles:r}=e,{div:a}=n.tags;Mt({css:o,createGlobalStyles:r});const s=o`
 ...
 `}),zt=e=>{const{bau:t}=e,{section:n,div:o,h1:r}=t.tags,a=Te(),s=Y(e),l=Nt(e);return function(){const i=t.state(!0),m=o(),d=c=>{m.replaceChildren(a({parent:m,animationHide:`${l.hideRight} 0.5s`,animationShow:`${l.showRight} 0.5s`},o(c.val?"Ciao":"")))};return d(i),n({id:"animate"},o(r("Test Animate"),o(s({onclick:()=>{i.val=!i.val,d(i)}},()=>i.val?"Hide":"Show")),m))}};function Be(e,t){const{bau:n}=e,{span:o,img:r}=n.tags,a=n.state(!0),s=n.state(!1),l=()=>a.val=!1,i=m=>{a.val=!1,s.val=!0};return function(...d){let[{color:c,variant:u="outline",size:h,width:g=60,height:p=60,...b},...v]=U(d);return o({class:$(t==null?void 0:t.class,b.class)},()=>a.val?"Loading...":"",()=>s.val&&"Error",r({width:g,height:p,onload:l,onerror:i,class:$(t==null?void 0:t.class,c,u,h,b.class),...b}))}}const Lt=e=>{const{tr:t,bau:n,css:o}=e,{section:r,h2:a,h3:s}=n.tags,l=o`
  ...
  `,h=n.state(""),g=n.state(""),p=n.state(!1),b=n.state(0),v=()=>{p.val=!1};return function(...E){let[{variant:S="outline",color:I,size:P,id:A,label:B,placeholder:M,Option:j,options:D,getOptionLabel:H=({label:G})=>G,...O},...W]=U(E);const f=n.state(D),x=()=>{R.openDialog(),p.val=!0,g.val="",f.val=D},k=()=>{R.closeDialog(),p.val=!1,g.val=""},C=G=>{const{value:V}=G.target;g.val=V,V?f.val=D.filter(K=>H(K).match(new RegExp(`${V}`,"i"))):f.val=D},T=G=>{console.log("onclickButton",p.val),p.val?k():x()},_=G=>V=>{h.val=H(G),k()},N=G=>{switch(G.key){case"Escape":k();break;case"ArrowDown":b.val++;break;case"ArrowUp":b.val--;break;case"Enter":h.val=H(f.val[b.val]),g.val="",k();break}},X=d({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":p,"aria-label":B,onclick:T,class:$(S,I,P)},()=>!h.val&&B,h),z=a({id:A,value:g,placeholder:M,autofocus:!0,type:"search",autocomplete:"new-password",autocapitalize:"none",spellcheck:!1,role:"combobox","aria-autocomplete":"list","aria-expanded":p,oninput:C,onkeydown:N,class:$(S,I,P)}),R=c({id:A,triggerEl:X,contentEl:(()=>r({class:$(S,I,P,"content")},z,()=>s(f.val.map((G,V)=>l({class:()=>$(b.val==V&&"active"),onclick:_(G)},j(G))))))(),onClose:v});return r({...O,class:$("autocomplete",u,t==null?void 0:t.class,O==null?void 0:O.class)},X,R)}}const Ht=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,span:i}=n.tags,m=(...g)=>a({class:o`
          display: flex;
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
  ...
  `;return function(...l){let[{color:i,variant:m="outline",size:d,content:c,...u},...h]=U(l);return r({...u,class:$("badge",a,t==null?void 0:t.class,u==null?void 0:u.class)},r({class:$(i,m,d)},c),...h)}}const Ot=e=>{const{bau:t,css:n}=e,{section:o,div:r,h3:a,h2:s}=t.tags,l=(...c)=>r({class:n`
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
...
`))()}
  ...
  `,d=n.state(""),c=n.state(!1),u=n.state(0);return function(...g){let[{color:p="neutral",variant:b="outline",size:v,id:w,label:E,Option:S,options:I,getOptionLabel:P=({label:C})=>C,...A},...B]=U(g);const M=()=>{k.openDialog(),c.val=!0},j=()=>{k.closeDialog(),c.val=!1},D=()=>{c.val=!1},H=C=>{c.val?j():M()},O=C=>T=>{d.val=P(C),j()},W=C=>{switch(C.preventDefault(),C.key){case"Escape":j();break;case"ArrowDown":u.val<I.length-1?u.val++:u.val=0;break;case"ArrowUp":u.val<=0?u.val=I.length-1:u.val--;break;case"Enter":c.val?(d.val=P(I[u.val]),j()):M();break}},f=()=>a(I.map((C,T)=>s({class:()=>$(u.val==T&&"active"),onclick:O(C)},S(C)))),x=l({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":c,"aria-label":E,onclick:H,class:$(p,b,v)},()=>!d.val&&E,d),k=i({id:w,triggerEl:x,contentEl:f(),onClose:D});return r({...A,class:$("select",m,t==null?void 0:t.class,A==null?void 0:A.class),onkeydown:W},x,k)}}const ea=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,span:i}=n.tags,m=(...g)=>a({class:o`
          display: flex;
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
...
`);function Fe(e,t){const{bau:n,css:o}=e,{input:r}=n.tags,a=o`
  ...
  `;return function(...l){let[{color:i="neutral",variant:m="plain",size:d,...c},...u]=U(l);return r({...c,class:$("switch",a,i,m,d,t==null?void 0:t.class,c.class),type:"checkbox",required:"required"},...u)}}const oa=e=>{const{tr:t,bau:n,css:o}=e,{section:r,form:a,label:s,div:l,h2:i}=n.tags,m=F(e),d=Fe(e);return()=>r({id:"switch"},i(t("Switch Examples")),a(l({class:o`
            ...
            `},s({for:"my-shinny-switch"},"My shinny switch"),d({id:"my-shinny-switch"}))),i(t("Switch Table")),m({Item:c=>l(s("off ",d({...c,id:`my-switch-example-off-${c.color}-${c.variant}`})),s("on ",d({...c,id:`my-switch-example-on-${c.color}-${c.variant}`,checked:!0})))}))};function de(e,t){const{bau:n,css:o}=e,{tabDefs:r}=t,{div:a,ul:s,li:l}=n.tags,i=n.state(r),m=n.state(r[0]),d=u=>i.val.find(h=>h.name==u),c={base:o`
      display: flex;
      flex-direction: column;
      & ul {
        display: flex;
...
`;const s=o`
  ...
  `,i=({label:h,icon:g,...p})=>a({variant:"plain","aria-label":h,title:h,...p},g),m=({count:h,totalCount:g,page:p,rowsPerPage:b})=>r({class:"pages-numbers"},Number(p-1)*Number(b)+(h>0?1:0),"-",Math.min(p*b,g)," of ",g),d=({count:h,page:g,rowsPerPage:p})=>r({class:"pages-numbers"},(g-1)*p+(h>0?1:0),"-",g*p),c=h=>h<=1,u=(h,g,p)=>h>=Math.ceil(g/p);return function(...g){let[{count:p=0,totalCount:b=0,page:v=1,rowsPerPage:w=50,onPageChange:E,isLoading:S=!1,disableFirst:I=()=>c(v),disablePrevious:P=()=>c(v),disableNext:A=()=>u(v,b,w),disableLast:B=()=>u(v,b,w),...M},...j]=U(g);const D=Math.max(0,Math.ceil(b/w)),H=E({page:1}),O=E({page:v-1}),W=E({page:v+1}),f=E({page:D});return r({...M,class:$("table-pagination",l,S&&"disabled",t==null?void 0:t.class,M==null?void 0:M.class)},s({class:"spinner",visibility:S,size:"md"}),b>0?m({count:p,totalCount:b,page:v,maxPages:D,rowsPerPage:w}):d({count:p,page:v,maxPages:D,rowsPerPage:w}),r(i({label:"First",icon:"⟪",onclick:H,disabled:I()}),i({label:"Previous",icon:"⟨",onclick:O,disabled:P()}),i({label:"Next",icon:"⟩",onclick:W,disabled:A()}),i({label:"Last",icon:"⟫",onclick:f,disabled:B()})))}}const la=(e=1e3)=>new Array(e).fill("").map(()=>({name:"my name",email:"myemail@mail.col"})),ia=e=>{const{bau:t,css:n}=e,{th:o,td:r,tr:a,table:s,thead:l,tbody:i}=t.tags,m=la(45),d=({name:w,email:E})=>a(r(w),r(E)),c=()=>l(o({class:n`
            text-align: left;
          `},"Name"),o({class:n`
            text-align: left;
          `},"Email")),u=Re(e),h=Q(e,{class:n`
      max-width: 650px;
    `}),g=t.state(m),p=t.state({count:m.length,totalCount:m.length,page:1,rowsPerPage:10}),b=t.derive(()=>g.val.slice(p.val.page*p.val.rowsPerPage,(p.val.page+1)*p.val.rowsPerPage)),v=({page:w})=>E=>{p.val.page=w};return()=>h(s(c(),()=>i(b.val.map(d))),()=>u({...p.val,onPageChange:v}))},ca=e=>{const{bau:t,css:n}=e,{th:o,td:r,tr:a,table:s,thead:l,tbody:i,div:m}=t.tags,d=t.state(!1),c=t.state([]),u=t.state(""),h=t.derive(()=>c.val.length),g=t.state(1),p=t.state(10),b=t.derive(()=>c.val),v=M=>`https://api.github.com/orgs/aws/repos?${new URLSearchParams(M).toString()}`,w=({page:M})=>j=>{g.val=M,E(v({page:M,per_page:p.val}))};E(v({page:1,per_page:p.val}));async function E(M){try{d.val=!0;const j=await fetch(M,{});if(j.ok){const D=await j.json();c.val=D;return}throw j}catch(j){u.val=j.message}finally{d.val=!1}}const S=({name:M,description:j,stargazers_count:D})=>a(r(M),r(j),r({class:n`
            text-align: right;
          `},D)),I=()=>l(o({class:n`
            text-align: left;
          `},"Name"),o({class:n`
      ...
      min-width: 650px;
    `}),B=({message:M})=>m(M);return()=>A(()=>P({rowsPerPage:p.val,page:g.val,count:h.val,totalCount:-1,isLoading:d.val,onPageChange:w,disableNext:()=>!1}),s(I(),()=>u.val&&B({message:u.val}),()=>i(b.val.map(S))))},da=e=>{const{bau:t,css:n}=e,{section:o,div:r,h3:a,h2:s,tr:l}=t.tags,i=ia(e),m=ca(e),d=(...c)=>r({class:n`
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
          & .table-container {
            width: 100%;
  ...
  `,expanded:e`
    ...
    `});function Ue(e,t){const{bau:n,css:o,createGlobalStyles:r,window:a}=e,{renderMenuItem:s}=t,{ul:l,li:i,nav:m,div:d}=n.tags,c=pa({css:o,createGlobalStyles:r}),u=({element:b,closeState:v})=>{b.scrollHeight!=0&&(v.val?h(b):g(b))};function h(b){b.style.height=b.scrollHeight+"px";const v=()=>{b.removeEventListener("transitionend",v)};b.addEventListener("transitionend",v),a.requestAnimationFrame(()=>{b.style.height="0px"})}function g(b){const v=()=>{b.removeEventListener("transitionend",v),b.style.height=null};b.addEventListener("transitionend",v),b.style.height=b.scrollHeight+"px"}const p=({depth:b=1,maxDepth:v})=>w=>{const{children:E,expanded:S}=w,I=n.state(!S);return i({class:()=>$(E?I.val?c.collapsed:c.expanded:"")},d({class:o`
        ...
        `},a({class:o`
          ...
          `},s(t("Pages Examples")),fa(e)()))}}const wa=e=>{const{bau:t,css:n,config:o}=e,{section:r,div:a,h1:s,span:l,p:i,ul:m,li:d,a:c,main:u,header:h,footer:g,label:p}=t.tags,{svg:b,use:v}=t.tagsNS("http://www.w3.org/2000/svg"),w=F(e),S=Me(e,{accordionDefs:[{name:"Item1",Header:()=>"Item 1",Content:()=>a(i("Item 1 Content"))},{name:"Item2",Header:()=>"Item 2",Content:()=>a(i("Item 2 Content"))},{name:"Item3",Header:()=>"Item 3",Content:()=>a(i("Item 3 content"))}]}),I=oe(e),P=De(e),A=[{code:"AD",label:"Andorra",phone:"376"},{code:"AE",label:"United Arab Emirates",phone:"971"},{code:"AF",label:"Afghanistan",phone:"93"}],B=y=>a({class:n`
          display: flex;
          justify-content: space-between;
          gap: 0.5rem;
        `},l(y.label),l(y.code)),M=Be(e,{class:n`
      > img {
        background: var(--color-gray-100);
        border-radius: 50%;
        margin: 0.3rem;
      }
    `}),j=ie(e),D={items:[{href:"/",name:"⌂"},{name:"Dir"},{href:"/dir/subdir",name:"SubDir"}]},H=Pe(e),O=Y(e),W=Ne(e),f=ce(e),x=Le(e),k=ze(e),C={data:{name:"Root Menu"},children:[{data:{name:"Menu 1",href:"#m1"},children:[{data:{name:"Sub Menu 1",href:"#menusub2"},children:[{data:{name:"Sub Sub Menu 1",href:"#menusubsub1"}}]},{data:{name:"Sub Menu 2",href:"#menusub1"}}]},{data:{name:"Menu 2",href:"#menu2"},children:[{data:{name:"Sub Menu 21",href:"#menusub21"}}]},{data:{name:"Menu 3",href:"#menu2"}}]},T=he(e,{base:o.base+"/components"}),_=({disabled:y})=>a({class:$(n`
            display: flex;
            align-items: center;
            flex-direction: column;
            stroke: var(--font-color-base);
          ...
          `,y&&n`
              color: var(--color-emphasis-500);
              fill: var(--font-color-disabled);
            `)},b({width:100,height:100,fill:"currentColor"},v({href:"uploadIcon.svg#Capa_1"})),l("Choose a file to upload")),N=He(e),X=Oe(e),z=je(e),L=()=>u(Array(10).fill("").map((y,q)=>i(q+1,". Some text here"))),R=y=>{const q=z({id:"my-dialog",...y},h("Header"),L(),g(O({variant:"outline",onclick:()=>{q.close()}},"Cancel"),O({variant:"solid",onclick:()=>{q.close()}},"OK")));return q},G=Ge(e),V=[{code:"AD",label:"Andorra",phone:"376"},{code:"AE",label:"United Arab Emirates",phone:"971"},{code:"AF",label:"Afghanistan",phone:"93"}],K=y=>a({class:n`
          display: flex;
          justify-content: space-between;
          gap: 0.5rem;
        `},l(y.label),l(y.code)),re=ne(e),_e=be(e),fe=Fe(e),Xe=de(e,{tabDefs:[{name:"Tab1",Header:()=>a("TAB"),Content:()=>a(i("My Tab 1 Content"))},{name:"Tab2",Header:()=>a("TAB 2"),Content:()=>a(i("My tab 2 Content"))},{name:"Tab Disabled",disabled:!0,Header:()=>a("Tab Disabled"),Content:()=>a(i("My tab Disabled"))}]}),We=Ve(e),qe=()=>l("My tooltip"),Ye=ue(e),Ke={data:{name:"Root Menu"},children:[{data:{name:"Menu 1",href:"#menu"},expanded:!0,children:[{data:{name:"Sub Menu 1",href:"#menusub2"}},{data:{name:"Sub Menu 2",href:"#menusub1"}}]},{data:{name:"Menu 2",href:"#menu2"},children:[{data:{name:"Sub Menu 21",href:"#menusub21"}}]}]},Ze=Ue(e,{renderMenuItem:({name:y,href:q})=>c({href:q,onclick:Je=>{Je.preventDefault()}},y)}),ve=[{name:"Accordion",Item:y=>S({...y})},{name:"Alert",Item:y=>I({...y},`Alert ${y.color}`)},{name:"Autocomplete",Item:y=>P({...y,options:A,Option:B,getOptionLabel:({label:q})=>q,label:"Country",placeholder:"Search countries",id:"country"})},{name:"Avatar",Item:y=>M({...y,src:"./grucloud.svg",alt:"GruCloud"})},{name:"Badge",Item:(y,{index:q})=>j({...y,content:`${q*100}`},"☏")},{name:"Breadcrumbs",Item:y=>H({...y,...D})},{name:"Button",Item:y=>O({...y},`${y.variant} ${y.color}`)},{name:"Button Group",Item:y=>W({...y},O({},"ONE"),O({},"TWO"),O({},"THREE"))},{name:"Calendar",Item:y=>f({...y})},{name:"Checkbox",Item:y=>x({id:`myCheckbox-${y.color}-${y.variant}`,name:`myCheckbox-${y.color}-${y.variant}`,...y})},{name:"Chip",Item:y=>k({...y},`Chip ${y.color}`)},{name:"DrillDown Menu",Item:y=>T({tree:C,...y})},{name:"File Input",Item:y=>N({Component:_,name:"file",accept:"text/*",onchange,...y})},{name:"Input",Item:y=>X({name:"my-input",id:"my-input-with",placeholder:"Enter text",...y})},{name:"Modal",Item:y=>{const q=R(y);return a(O({...y,onclick:()=>{q.showModal()}},"OPEN MODAL"),q)}},{name:"Select",Item:y=>a(G({...y,options:V,Option:K,getOptionLabel:({label:q})=>q,label:"Select a country..."}))},{name:"Slider",Item:y=>re({...y,id:`mySwitch-${y.color}-${y.variant}`})},{name:"Spinner",Item:y=>_e(y)},{name:"Switch",Item:y=>a(p(fe({...y,id:`mySwitch-off-${y.color}-${y.variant}`})),p(fe({...y,id:`mySwitch-on-${y.color}-${y.variant}`,checked:!0})))},{name:"Tabs",Item:y=>Xe(y)},{name:"Theme Switch",Item:y=>We(y)},{name:"Tooltip",Item:y=>Ye({titleEl:qe(),...y},O({},`${y.color} ${y.variant}`))},{name:"Tree View",Item:y=>Ze({...y,tree:Ke})}];return()=>r(s("Bau Component Gallery"),i("This page displays the components with various colors and variants."),m({class:n`
            display: inline-flex;
            flex-wrap: wrap;
            gap: 1rem;
            list-style: none;
            padding: 0;
          `},ve.map(({name:y})=>d(k({color:"primary"},c({href:`#${y}`},y))))),ve.map(y=>a({id:y.name,class:n`
              border: 1px dotted var(--color-emphasis-400);
              padding: 1rem;
              margin: 1rem;
            `},w(y))))},ya=({context:e})=>[{path:"",action:t=>({title:"Bau UI",component:Tt(e)})},{path:"components",action:()=>({title:"Component",component:wa(e)}),children:[{path:"accordion",action:()=>({title:"Accordion",component:At(e)})},{path:"alert",action:()=>({title:"Alert",component:Bt(e)})},{path:"alertStack",action:()=>({title:"Alert Stack",component:Pt(e)})},{path:"animate",action:()=>({title:"Animate",component:zt(e)})},{path:"autocomplete",action:()=>({title:"Autocomplete",component:Ht(e)})},{path:"avatar",action:()=>({title:"Avatar",component:Lt(e)})},{path:"badge",action:()=>({title:"Badge",component:Ot(e)})},{path:"breadcrumb",action:()=>({title:"Breadcrumb",component:jt(e)})},{path:"button",action:()=>({title:"Button",component:Gt(e)})},{path:"buttonGroup",action:()=>({title:"Button Group",component:Rt(e)})},{path:"calendar",action:()=>({title:"Calendar",component:Vt(e)})},{path:"chip",action:()=>({title:"Chip",component:Ut(e)})},{path:"checkbox",action:()=>({title:"Checkbox",component:_t(e)})},{path:"drawer",action:()=>({title:"Drawer",component:Wt(e)})},{path:"drillDownMenu",action:()=>({title:"DrillDown Menu",component:qt(e)})},{path:"fileInput",action:()=>({title:"File Input",component:Yt(e)})},{path:"input",action:()=>({title:"Input",component:Zt(e)})},{path:"modal",action:()=>({title:"Modal",component:Jt(e)})},{path:"popover",action:()=>({title:"Popover",component:Qt(e)})},{path:"select",action:()=>({title:"Select",component:ea(e)})},{path:"slider",action:()=>({title:"Slider",component:ta(e)})},{path:"spinner",action:()=>({title:"Spinner",component:aa(e)})},{path:"switch",action:()=>({title:"Switch",component:oa(e)})},{path:"table",action:()=>({title:"Table",component:sa(e)})},{path:"tablePagination",action:()=>({title:"Table Pagination",component:da(e)})},{path:"tabs",action:()=>({title:"Tabs",component:ra(e)})},{path:"tooltip",action:()=>({title:"Tooltip",component:ua(e)})},{path:"themeSwitch",action:()=>({title:"Theme Switch",component:ha(e)})},{path:"treeView",action:()=>({title:"Tree View",component:ba(e)})}]},{path:"pages",action:t=>({title:"Pages",component:va(e)})}],xa=e=>({title:e.tr("Page Not Found"),component:()=>"Not Found"}),ka=({context:e,LayoutDefault:t,config:{base:n=""}})=>{const{window:o,bau:r,states:a}=e,s=r.state(),l=t({componentState:s});return document.getElementById("app").replaceChildren(l),({router:m})=>{const d=o.location.pathname.replace(n,""),{title:c,component:u,Layout:h=t}=m.resolve({pathname:d});a.pathname.val=d,s.val=u,document.title=`${c}`}},Ca=e=>{const{createGlobalStyles:t}=e;ct(e,{colorPalette:[["neutral",{h:"0",s:"0%",l:"40%"}],["primary",{h:"230",s:"48%",l:"47%"}],["secondary",{h:"338",s:"100%",l:"48%"}],["success",{h:"120",s:"100%",l:"32%"}],["info",{h:"194",s:"80%",l:"62%"}],["warning",{h:"43",s:"100%",l:"50%"}],["danger",{h:"358",s:"95%",l:"60%"}]]}),t`
    :root {
      --header-height: 3rem;
    }
    html {
