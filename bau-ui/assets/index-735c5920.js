...
`)}
  ...
  `}function pt(){const e=document.getElementById("loading");e&&e.classList.add("m-fadeOut")}let me=e=>Object.prototype.toString.call(e??0).slice(8,-1),bt=e=>me(e)=="Object",ke=e=>me(e)=="Function",se=e=>["Object","Array"].includes(me(e)),Se=Object.getPrototypeOf,le=e=>ee(e)?e.val:e,ee=e=>e==null?void 0:e.__isState,gt=["splice","push","pop","shift","unshift","sort","reverse"],te=(e,t)=>{const n=new Array(e.length);for(let o=0;o<e.length;o++)n[o]=t(e[o],o);return n};const U=e=>!ee(e[0])&&bt(e[0])?e:[{},...e];function ft(e){let t=(e==null?void 0:e.window)??window,{document:n}=t,o,r=new Set,a=new Set,s=!1,l,c=f=>n.createElement(f),m=(f,x,k)=>{let C=l;l=x;let E=f(k);return l=C,E},d=()=>{o||(o=window.requestAnimationFrame(()=>{r.forEach(f=>{f.bindings=f.bindings.filter(x=>{var k;return(k=x.element)==null?void 0:k.isConnected}),!f.bindings.length&&!f.computed&&r.delete(f)}),o=void 0}))},i=(f,x,k,C,E,_)=>{var z;if(s){a.add(f);return}for(let R of f.bindings){let{deps:j,element:L,renderInferred:O,render:X,renderItem:q}=R;if(q&&x)(z=b(L,C,(...Z)=>h(q(...Z)),k,E,_)[x])==null||z.call();else{let Z=O?O({element:L}):X({element:L,renderItem:q})(...j.map(le));Z!==L&&L.replaceWith(R.element=h(Z))}}$(f),d()},u=(f,x,k=[])=>({get(C,E,_){var z;if(l==null||l.add(f),E==="_isProxy")return!0;if(!((z=C[E])!=null&&z._isProxy)&&!ee(C[E])&&se(C[E]))C[E]=new Proxy(C[E],u(f,x,[...k,E]));else if(gt.includes(E)){let R=C[E];return(...j)=>{let L=R.apply(C,j);return i(f,E,L,j,x,k),L}}return Reflect.get(C,E,_)},set(C,E,_,z){let R=Reflect.set(C,E,_,z);return i(f,"setItem",R,{prop:E,value:_},x,[...k,E]),R}}),g=(f,x)=>new Proxy(x,u(f,x)),b=(f,x,k,C,E,_)=>{let z=()=>f.replaceChildren(...te(C,k)),R=j=>f[j]&&f.removeChild(f[j]);return{assign:z,sort:z,reverse:z,setItem:()=>{var L;let j=_[0];(L=f.children[j])==null||L.replaceWith(k(E[j],j))},push:()=>f.append(...te(x,(j,L)=>k(j,E.length+L))),unshift:()=>f.prepend(...te(x,k)),pop:()=>R("lastChild"),shift:()=>R("firstChild"),splice:()=>{let[j,L,...O]=x;const{length:X}=f.children;for(let q=j>=0?Math.min(j+L-1,X-1):X-1;q>=(j>=0?j:X+j);q--)f.children[q].remove();if(O.length){let q=O.forEach((Z,re)=>k(Z,j+re));f.children[j]?f.children[j].after(...q):f.append(...q)}}}},p=f=>({oldVal:f,bindings:[],listeners:[],__isState:!0,get val(){let x=this;return l==null||l.add(x),x.valProxy??(x.valProxy=se(f)?g(x,f):f,x.valProxy)},set val(x){let k=this,C=k.val;se(x)?(k.valProxy=g(k,x),i(k,"assign",x)):x!==C&&(k.valProxy=x,i(k)),k.oldVal=C}}),h=f=>f==null||f===!1?c("span"):f.nodeType?f:n.createTextNode(f),v=(f,x)=>{let k=new Set;return x.val=m(f,k),k},y=f=>{let x=p(),k=v(f,x);x.computed=!0;for(let C of k)C.listeners.push({computed:f,deps:k,state:x});return x},$=f=>{for(let x of[...f.listeners])v(x.computed,x.state)},S=(f,...x)=>{if(x.length){let k=[];for(let C of x.flat(1/0))C!=null&&k.push(ee(C)?N({deps:[C],render:()=>E=>E}):ke(C)?F({renderInferred:C}):h(C));f.append(...k)}},M={},D=(f,x)=>f&&(Object.getOwnPropertyDescriptor(f,x)??D(Se(f),x)),A=(f,x,k)=>{var C;return M[f+","+x]??(M[f+","+x]=((C=D(k,x))==null?void 0:C.set)??0)},I=(f,x)=>new MutationObserver((k,C)=>{k.filter(E=>E.removedNodes).forEach(E=>[...E.removedNodes].find(_=>_===f&&(x({element:f}),C.disconnect(),!0)))}).observe(f.parentNode,{childList:!0}),B=f=>new Proxy(function(k,...C){var R;let[E,..._]=U(C),z=f?n.createElementNS(f,k):c(k);for(let[j,L]of Object.entries(E)){if(j.startsWith("bau"))continue;let O=A(k,j,Se(z))?X=>z[j]=X:X=>z.setAttribute(j,X);L==null||(ee(L)?N({deps:[L],render:()=>()=>(O(L.val),z)}):ke(L)&&(!j.startsWith("on")||L.isDerived)?F({renderInferred:()=>(O(L({element:z})),z)}):L.renderProp?N({deps:L.deps,render:()=>()=>(O(L.renderProp({element:z})(...L.deps.map(le))),z)}):O(L))}return S(z,..._),(R=E.bauCreated)==null||R.call(E,{element:z}),E.bauMounted&&t.requestAnimationFrame(()=>E.bauMounted({element:z})),E.bauUnmounted&&t.requestAnimationFrame(()=>I(z,E.bauUnmounted)),z},{get:(x,k)=>x.bind(void 0,k)}),P=(f,x,k)=>{f.element=h(k);for(let C of x)ee(C)&&(r.add(C),C.bindings.push(f));return f.element},F=({renderInferred:f,element:x})=>{let k=new Set,C=m(f,k,{element:x});return P({renderInferred:f},k,C)},N=({deps:f,element:x,render:k,renderItem:C})=>P({deps:f,render:k,renderItem:C},f,k({element:x,renderItem:C})(...f.map(le))),H=(f,x,k)=>N({deps:[f],render:({renderItem:C})=>E=>(x.append(...te(E,C)),x),renderItem:k}),G=f=>{s=!0,f(),s=!1,a.forEach(i),a.clear()};return{tags:B(),tagsNS:B,state:p,bind:N,loop:H,derive:y,stateSet:r,batch:G}}const vt=e=>{let t=0,n=11;for(;t<e.length;)n=101*n+e.charCodeAt(t++)>>>0;return"bau"+n},wt=(e,t,n,o)=>{const r=e.createElement("style");r.id=n,r.append(o),(t??e.head).append(r)},yt=(e,t)=>e.reduce((n,o,r)=>n+o+(t[r]??""),"");function xt(e){let{document:t}=(e==null?void 0:e.window)??window;const n=o=>(r,...a)=>{const s=yt(r,a),l=vt(s);return!t.getElementById(l)&&wt(t,e==null?void 0:e.target,l,o(l,s)),l};return{css:n((o,r)=>`.${o} { ${r} }`),keyframes:n((o,r)=>`@keyframes ${o} { ${r} }`),createGlobalStyles:n((o,r)=>r)}}function Ct(e){return{bau:ft(),...xt(),tr:n=>n,window,...e}}function T(...e){return e.filter(t=>t).join(" ")}function W(e,t){const{bau:n,css:o}=e,r={root:o`
      color: inherit;
      display: inline-flex;
      align-items: center;
...
`))()}
 ...
 `});function pe(e,t){const{bau:n,css:o,window:r}=e,{base:a=""}=t,s=({currentTree:N,data:H,onclickBack:G})=>h(S({variant:"plain",href:`${a}${N.parentTree.children[0].data.href}`,onclick:G({currentTree:N}),class:o`
  ...
  `,P=({variant:N,color:H,size:G,onclickItem:f,onclickBack:x,currentTree:k,pathnameState:C})=>{const{children:E,parentTree:_,data:z}=k;return p({class:T("drillDownMenu",N,H,G)},_&&m({data:z,currentTree:k,onclickBack:x}),E&&$({class:T(N,H,G)},E.map(R=>g({class:()=>T(R.children&&"has-children",i({pathname:C.val,subTree:R})&&"active"),onclick:R.children&&f({currentTree:R})},d(R)))))},F=({tree:N,pathname:H})=>{let G=Le({})(N),f=Pe(H)(G);return f||(console.log("drilldown no sub tree",H),f=G),f};return function(H){const{variant:G="plain",color:f="neutral",size:x,tree:k,pathnameState:C=n.state(r.location.pathname),...E}=H,_=({currentTree:L})=>O=>R(O,j,L,!0),z=({currentTree:L})=>O=>R(O,j,L.parentTree,!1),R=(L,O,X,q)=>{O.firstChild.replaceChildren(y({parent:O,animationHide:`${q?M:D} ${$e}`,animationShow:`${q?A:I} ${$e}`},P({variant:G,color:f,size:x,currentTree:X,onclickItem:_,onclickBack:z,pathnameState:C})))},j=b({class:T(B,t==null?void 0:t.class,E.class)},()=>P({variant:G,color:f,size:x,currentTree:F({tree:k,pathname:C.val}),onclickItem:_,onclickBack:z,pathnameState:C}));return j}}const It={data:{name:"Root"},children:[{data:{name:"Bau UI",href:"/"}},{data:{name:"Components",href:"/components"},children:[{data:{name:"Accordion",href:"/components/accordion"}},{data:{name:"Alert",href:"/components/alert"}},{data:{name:"Alert Stack",href:"/components/alertStack"}},{data:{name:"Animate",href:"/components/animate"}},{data:{name:"Autocomplete",href:"/components/autocomplete"}},{data:{name:"Avatar",href:"/components/avatar"}},{data:{name:"Badge",href:"/components/badge"}},{data:{name:"Breadcrumb",href:"/components/breadcrumb"}},{data:{name:"Button",href:"/components/button"}},{data:{name:"Button Group",href:"/components/buttonGroup"}},{data:{name:"Calendar",href:"/components/calendar"}},{data:{name:"Chip",href:"/components/chip"}},{data:{name:"Checkbox",href:"/components/checkbox"}},{data:{name:"Drawer",href:"/components/drawer"}},{data:{name:"DrillDown Menu",href:"/components/drillDownMenu"}},{data:{name:"File Input",href:"/components/fileInput"}},{data:{name:"Input",href:"/components/input"}},{data:{name:"List",href:"/components/list"}},{data:{name:"Modal",href:"/components/modal"}},{data:{name:"Popover",href:"/components/popover"}},{data:{name:"Select",href:"/components/select"}},{data:{name:"Slider",href:"/components/slider"}},{data:{name:"Spinner",href:"/components/spinner"}},{data:{name:"Switch",href:"/components/switch"}},{data:{name:"Table",href:"/components/table"}},{data:{name:"Table Pagination",href:"/components/tablePagination"}},{data:{name:"Tab",href:"/components/tabs"}},{data:{name:"Tooltip",href:"/components/tooltip"}},{data:{name:"Theme Switch",href:"/components/themeSwitch"}},{data:{name:"Tree View",href:"/components/treeView"}}]}]};function Ne(e){const{tr:t,bau:n,css:o,config:r,states:a,window:s}=e,{div:l,ul:c,li:m,nav:d,a:i,span:u}=n.tags;let g=!1;const b=pe(e,{base:r.base});return function(){return l({bauMounted:({element:h})=>{s.innerWidth<=640&&(g=!0,a.drawerOpen.val=!1)},onclick:h=>{g&&!h.target.dataset.buttonback&&!h.target.parentElement.classList.contains("has-children")&&(a.drawerOpen.val=!1)},style:()=>a.drawerOpen.val?"display:block;":"display:none;",class:T(o`
            grid-area: sidebar;
            position: sticky;
          ...
          `)},b({tree:It,pathnameState:a.pathname}))}}const Mt=e=>{const{bau:t,css:n,states:o}=e,{div:r}=t.tags,a=Et(e),s=Ne(e),l=Tt(e);return function({componentState:m}){return r({class:n`
          display: grid;
...
`);function ze(e,t){const{bau:n,css:o}=e,{accordionDefs:r}=t,{div:a,ul:s,li:l,header:c,h3:m,button:d}=n.tags,i=n.state(""),u=p=>h=>{i.val==p?i.val="":i.val=p},g=({element:p,open:h})=>{const v=()=>{p.removeEventListener("transitionend",v)};function y(S){S.addEventListener("transitionend",v),window.requestAnimationFrame(()=>{S.style.height="0px"})}function $(S){S.addEventListener("transitionend",v),S.style.height=S.scrollHeight+"px"}p.scrollHeight!=0&&(h?$(p):y(p))},b=o`
    ...
    ${Pt()}
  `;return function(...h){let[{color:v,variant:y="outline",size:$,content:S,...M},...D]=U(h);const A=I=>{const{Header:B,Content:P,name:F}=I;return l({class:T(v,y,$),onclick:u(F)},m({class:()=>T(i.val==F&&"active")},d({type:"button","aria-controls":`bau-${F}`,"aria-expanded":({element:N})=>i.val==F},B(I))),a({class:"content",role:"region",id:`bau-${F}`,"data-state":({element:N})=>{const H=i.val==F;return g({element:N,open:H}),H}},P(I)))};return a({class:T("accordion",b,t==null?void 0:t.class,M.class)},s(r.map(A)))}}const V=e=>{const{bau:t,css:n}=e,{div:o,table:r,tbody:a,tr:s,td:l,thead:c,th:m}=t.tags;return function({Item:i,name:u}){return o({class:n`
          overflow-x: auto;
          overflow-y: visible;
          width: 100%;
          max-width: 80vw;
...
`);function He(e,t){const{bau:n,css:o}=e,{div:r,li:a,ul:s}=n.tags,l=be(e),c=W(e),m=ge(e),d=oe(e),i=o`
    ...
    ${Xt()}
  `,u=n.state(""),g=n.state(""),b=n.state(!1),p=n.state(0),h=()=>{b.val=!1};return function(...y){let[{variant:$="outline",color:S,size:M,id:D,label:A,placeholder:I,Option:B,options:P,getOptionLabel:F=({label:O})=>O,...N},...H]=U(y);const G=n.state(P),f=()=>{L.openDialog(),b.val=!0,g.val="",G.val=P},x=()=>{L.closeDialog(),b.val=!1,g.val=""},k=O=>{const{value:X}=O.target;g.val=X,X?G.val=P.filter(q=>F(q).match(new RegExp(`${X}`,"i"))):G.val=P},C=O=>{b.val?x():f()},E=({option:O,index:X})=>q=>{u.val=F(O),p.val=X,x()},_=O=>{switch(console.log("onkeydown",O.key,p.val),O.key){case"Escape":x();break;case"ArrowDown":p.val<G.val.length-1?p.val++:p.val=0;break;case"ArrowUp":p.val<=0?p.val=G.val.length-1:p.val--;break;case"Enter":u.val=F(G.val[p.val]),g.val="",x();break}},z=c({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":b,"aria-label":A,onclick:C,variant:$,color:S,size:M},()=>!u.val&&A,u),R=m({id:D,value:g,placeholder:I,autofocus:!0,type:"search",autocomplete:"new-password",autocapitalize:"none",spellcheck:!1,role:"combobox","aria-autocomplete":"list","aria-expanded":b,oninput:k,onkeydown:_,variant:$,color:S,size:M}),L=l({id:D,triggerEl:z,contentEl:(()=>r({class:T($,S,M,"content")},R,()=>d({class:T($,S,M)},G.val.map((O,X)=>a({class:()=>T(p.val==X&&"active"),onclick:E({option:O,index:X})},B(O))))))(),onClose:h});return r({...N,class:T("autocomplete",i,t==null?void 0:t.class,N==null?void 0:N.class)},z,L)}}const Wt=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,span:c}=n.tags,m=(...b)=>a({class:o`
          display: flex;
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
...
`);function Ue(e,t){const{bau:n,css:o}=e,{div:r,li:a}=n.tags,s=W(e),l=be(e),c=oe(e),m=o`
    ...
    ${ca()}
  `,d=n.state(""),i=n.state(!1),u=n.state(0);return function(...b){let[{color:p="neutral",variant:h="outline",size:v,id:y,label:$,Option:S,options:M,getOptionLabel:D=({label:C})=>C,...A},...I]=U(b);const B=()=>{k.openDialog(),i.val=!0},P=()=>{k.closeDialog(),i.val=!1},F=()=>{i.val=!1},N=C=>{i.val?P():B()},H=({option:C,index:E})=>_=>{d.val=D(C),u.val=E,P()},G=C=>{switch(C.preventDefault(),C.key){case"Escape":P();break;case"ArrowDown":u.val<M.length-1?u.val++:u.val=0;break;case"ArrowUp":u.val<=0?u.val=M.length-1:u.val--;break;case"Enter":i.val?(d.val=D(M[u.val]),P()):B();break}},f=()=>c({class:T(p,h)},M.map((C,E)=>a({class:()=>T(u.val==E&&"active"),onclick:H({option:C,index:E})},S(C)))),x=s({type:"button",role:"combobox","aria-autocomplete":"list","aria-expanded":i,"aria-label":$,onclick:N,color:p,variant:h,size:v},()=>!d.val&&$,d),k=l({id:y,triggerEl:x,contentEl:f(),onClose:F});return r({...A,class:T("select",p,v,m,t==null?void 0:t.class,A==null?void 0:A.class),onkeydown:G},x,k)}}const da=e=>{const{tr:t,bau:n,css:o}=e,{section:r,div:a,h3:s,h2:l,span:c}=n.tags,m=(...b)=>a({class:o`
          display: flex;
          border: 1px dotted var(--color-gray-500);
          padding: 1rem;
...
`;const s=o`
  ...
  `,m=({label:b,icon:p,...h})=>s({"aria-label":b,title:b,...h},p),d=({count:b,totalCount:p,page:h,rowsPerPage:v})=>r({class:"pages-numbers"},Number(h-1)*Number(v)+(b>0?1:0),"-",Math.min(h*v,p)," of ",p),i=({count:b,page:p,rowsPerPage:h})=>r({class:"pages-numbers"},(p-1)*h+(b>0?1:0),"-",p*h),u=b=>b<=1,g=(b,p,h)=>b>=Math.ceil(p/h);return function(...p){let[{count:h=0,totalCount:v=0,page:y=1,rowsPerPage:$=50,onPageChange:S,isLoading:M=!1,disableFirst:D=()=>u(y),disablePrevious:A=()=>u(y),disableNext:I=()=>g(y,v,$),disableLast:B=()=>g(y,v,$),...P},...F]=U(p);const N=Math.max(0,Math.ceil(v/$)),H=S({page:1}),G=S({page:y-1}),f=S({page:y+1}),x=S({page:N}),k=[{label:"First",icon:"⟪",onclick:H,disabled:D()},{label:"Previous",icon:"⟨",onclick:G,disabled:A()},{label:"Next",icon:"⟩",onclick:f,disabled:I()},{label:"Last",icon:"⟫",onclick:x,disabled:B()}];return r({...P,class:T("table-pagination",c,M&&"disabled",t==null?void 0:t.class,P==null?void 0:P.class)},l({class:"spinner",visibility:M,size:"md"}),v>0?d({count:h,totalCount:v,page:y,maxPages:N,rowsPerPage:$}):i({count:h,page:y,maxPages:N,rowsPerPage:$}),a({variant:"outline",color:"neutral"},k.map(C=>m({...C,variant:"outline",color:"neutral"}))))}}const va=(e=1e3)=>new Array(e).fill("").map(()=>({name:"my name",email:"myemail@mail.col"})),wa=e=>{const{bau:t,css:n}=e,{th:o,td:r,tr:a,table:s,thead:l,tbody:c}=t.tags,m=va(45),d=({name:y,email:$})=>a(r(y),r($)),i=()=>l(o({class:n`
            text-align: left;
          `},"Name"),o({class:n`
            text-align: left;
          `},"Email")),u=Xe(e),g=Q(e,{class:n`
      max-width: 650px;
    `}),b=t.state(m),p=t.state({count:m.length,totalCount:m.length,page:1,rowsPerPage:10}),h=t.derive(()=>b.val.slice(p.val.page*p.val.rowsPerPage,(p.val.page+1)*p.val.rowsPerPage)),v=({page:y})=>$=>{p.val.page=y};return()=>g(s(i(),()=>c(h.val.map(d))),()=>u({...p.val,onPageChange:v}))},ya=e=>{const{bau:t,css:n}=e,{th:o,td:r,tr:a,table:s,thead:l,tbody:c,div:m}=t.tags,d=t.state(!1),i=t.state([]),u=t.state(""),g=t.derive(()=>i.val.length),b=t.state(1),p=t.state(10),h=t.derive(()=>i.val),v=B=>`https://api.github.com/orgs/aws/repos?${new URLSearchParams(B).toString()}`,y=({page:B})=>P=>{b.val=B,$(v({page:B,per_page:p.val}))};$(v({page:1,per_page:p.val}));async function $(B){try{d.val=!0;const P=await fetch(B,{});if(P.ok){const F=await P.json();i.val=F;return}throw P}catch(P){u.val=P.message}finally{d.val=!1}}const S=({name:B,description:P,stargazers_count:F})=>a(r(B),r(P),r({class:n`
            text-align: right;
          `},F)),M=()=>l(o({class:n`
            text-align: left;
          `},"Name"),o({class:n`
